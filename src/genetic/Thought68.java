package genetic;
import java.util.ArrayList;
class Thought68 extends Thought{
private static ArrayList<Thought68> instances = new ArrayList<>();
private static int instanceCounter = 0;
private boolean fb0 = true;
private boolean fb1 = false;
private double fd0 = 731.2903675153071;
private double fd1 = 741.9307320708124;
private Thought fo0 = null;
private Thought fo1 = null;
Thought68 (){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
Thought.STACK_COUNTER++;
}
Thought68 (boolean ab1, boolean ab2, boolean ab3, boolean ab4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fb0 = ab1; 
fb1 = ab2; 
Thought.STACK_COUNTER++;
}
Thought68 (double ad1, double ad2, double ad3, double ad4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fd0 = ad1; 
fd1 = ad2; 
Thought.STACK_COUNTER++;
}
Thought68 (double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fb0 = ab1; 
fb1 = ab2; 
fd0 = ad1; 
fd1 = ad2; 
Thought.STACK_COUNTER++;
}
Thought68 (Thought ao1, Thought ao2, Thought ao3, Thought ao4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fo0 = ao1; 
fo1 = ao2; 
Thought.STACK_COUNTER++;
}
Thought68 (Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fd0 = ad1; 
fd1 = ad2; 
fo0 = ao1; 
fo1 = ao2; 
Thought.STACK_COUNTER++;
}
Thought68 (Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fb0 = ab1; 
fb1 = ab2; 
fo0 = ao1; 
fo1 = ao2; 
Thought.STACK_COUNTER++;
}
Thought68 (Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fb0 = ab1; 
fb1 = ab2; 
fd0 = ad1; 
fd1 = ad2; 
fo0 = ao1; 
fo1 = ao2; 
Thought.STACK_COUNTER++;
}
public static Thought68 getInstance(){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought68 instance = new Thought68 (); 
instances.add(instance); 
return instance; 
} 
}
public static Thought68 getInstance(boolean ab1, boolean ab2, boolean ab3, boolean ab4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought68 instance = new Thought68 ( ab1,  ab2,  ab3,  ab4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought68 getInstance(double ad1, double ad2, double ad3, double ad4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought68 instance = new Thought68 ( ad1,  ad2,  ad3,  ad4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought68 getInstance(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought68 instance = new Thought68 ( ad1,  ad2,  ad3,  ad4,  ab1,  ab2,  ab3,  ab4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought68 getInstance(Thought ao1, Thought ao2, Thought ao3, Thought ao4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought68 instance = new Thought68 ( ao1,  ao2,  ao3,  ao4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought68 getInstance(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought68 instance = new Thought68 ( ao1,  ao2,  ao3,  ao4,  ad1,  ad2,  ad3,  ad4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought68 getInstance(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought68 instance = new Thought68 ( ao1,  ao2,  ao3,  ao4,  ab1,  ab2,  ab3,  ab4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought68 getInstance(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought68 instance = new Thought68 ( ao1,  ao2,  ao3,  ao4,  ad1,  ad2,  ad3,  ad4,  ab1,  ab2,  ab3,  ab4); 
instances.add(instance); 
return instance; 
} 
}
void m1() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb0 = fb1 || fb0;
    boolean lb0 = true;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, fd1, fd0, fd1, fd0);
}
if(fo1 != null){
      fo1.m3(fb0, fb1, lb0, fb0);
}
    Thought lo1 = Thought59.getInstance();
    Thought lo2 = Thought49.getInstance(fd1, fd0, fd1, fd0, fb1, lb0, fb0, fb1);
    fd1 = fd0 - fd1;
    fd0 = fd1 - fd0;
    for(int i0=0; i0<10; i0++){
        fd1 = fd0 - fd1;
}
Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ab1 = ab2 || ab3;
    boolean lb0 = true;
    ab3 = ab4 && fb0;
if(fo0 != null){
      fd0 = fo0.m3(fo1, fo0, fo1, fo0);
}
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, fd1, fd0, fd1, fd0, fb1, lb0, ab1, ab2);
}
    ab3 = ab4 && fb0;
    boolean lb1 = false;
    if (fb0) {
        fd1 = fd0 - fd1;
        fb1 = lb0 && lb1;
if(fo1 != null){
          fd0 = fo1.m3(fo0, fo1, fo0, fo1, ab1, ab2, ab3, ab4);
}
        Thought lo2 = Thought21.getInstance(fd1, fd0, fd1, fd0);
if(fo0 != null){
          fd1 = fo0.m3(fo1, fo0, fo1, fo0, fd0, fd1, fd0, fd1);
}
if(fo0 != null){
          fo1 = fo0.m4(fb0, fb1, lb0, lb1);
}
        ab1 = fd0 < fd1;
        ab2 = ab3 && ab4;
if(fo0 != null){
          fo1 = fo0.m4();
}
        fb0 = fd0 < fd1;
        fd0 = fd1 - fd0;
        fb1 = fd1 < fd0;
        lb0 = lb1 || ab1;
        boolean lb3 = false;
        ab1 = fd1 > fd0;
        Output.points[2][4] -= fd1;
if(fo0 != null){
          fo1 = fo0.m4(fd0, fd1, fd0, fd1, ab2, ab3, ab4, fb0);
}
        boolean lb4 = false;
if(fo1 != null){
          fo1.m2(fo0, fo1, fo0, fo1);
}
}
Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ad1 *= -1;
    fb0 = ad2 < ad3;
    ad4 = fd0 - fd1;
    boolean lb0 = false;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, ad1, ad2, ad3, ad4, fb0, fb1, lb0, fb0);
}
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, fb1, lb0, fb0, fb1);
}
if(fo0 != null){
      fo0.m1(fd0, fd1, ad1, ad2);
}
    lb0 = ad3 > ad4;
    fb0 = fd0 < fd1;
    fb1 = lb0 && fb0;
    Thought lo1 = Thought31.getInstance(fo1, fo0, fo1, fo0, ad1, ad2, ad3, ad4);
    fd0 = fd1 - ad1;
    ad2 = ad3 + ad4;
if(fo0 != null){
      fo1 = fo0.m4(fb1, lb0, fb0, fb1);
}
    fd0 = fd1 - ad1;
    Output.points[2][5] += ad2;
    lb0 = !fb0;
    ad3 *= -1;
if(fo1 != null){
      ad4 = fo1.m3();
}
if(fo0 != null){
      fd0 = fo0.m3(fd1, ad1, ad2, ad3, fb1, lb0, fb0, fb1);
}

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    boolean lb0 = false;
    lb0 = !ab1;
    ab2 = ab3 || ab4;
        Output.points[2][6] -= ad2;
if(fo1 != null){
      fo1.m3(fo0, fo1, fo0, fo1);
}
    fb0 = ad3 > ad4;
    fd0 = fd1 + ad1;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, ad2, ad3, ad4, fd0, fb1, lb0, ab1, ab2);
}
    Thought lo1 = Thought98.getInstance(fo0, fo1, fo0, fo1, ab3, ab4, fb0, fb1);
    Output.points[2][7] -= fd1;
    ad1 = ad2 - ad3;
    lb0 = ad4 < fd0;
if(fo1 != null){
      fo0 = fo1.m4(fd1, ad1, ad2, ad3);
}
if(fo0 != null){
      ad4 = fo0.m3(fo1, fo0, fo1, fo0, fd0, fd1, ad1, ad2);
}
    double ld2 = 451.61773185407884;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(Thought ao1, Thought ao2, Thought ao3, Thought ao4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao3 != null){
      ao2 = ao3.m4(fb1, fb0, fb1, fb0);
}
    fd0 *= -1;
    boolean lb0 = true;
if(ao4 != null){
      fb0 = ao4.m2();
}
if(fo1 != null){
      fo0 = fo1.m4(fd1, fd0, fd1, fd0, fb1, lb0, fb0, fb1);
}
    lb0 = fd1 < fd0;
    for(int i0=0; i0<10; i0++){
        fb0 = fb1 || lb0;
        Thought lo1 = Thought45.getInstance(ao1, ao2, ao3, ao4);
        boolean lb2 = false;
if(fo0 != null){
          fo0.m3(fo1, ao1, ao2, ao3, fd1, fd0, fd1, fd0, lb0, fb0, fb1, lb2);
}
if(ao4 != null){
          ao4.m2(fo0, fo1, ao1, ao2, lb0, fb0, fb1, lb2);
}
        boolean lb3 = false;
        Output.points[2][8] += fd1;
        fd0 = fd1 - fd0;
        double ld4 = 705.8530503708365;
if(ao4 != null){
          ao3 = ao4.m4(fd0, fd1, ld4, fd0);
}
if(fo0 != null){
          fd1 = fo0.m3(fo1, ao1, ao2, ao3, ld4, fd0, fd1, ld4);
}
        fd0 = fd1 + ld4;
        lb3 = lb0 && fb0;
        double ld5 = 566.2216301480789;
}
Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao3 != null){
      ao2 = ao3.m4(fb1, fb0, fb1, fb0);
}
    fb1 = ad2 < ad3;
if(fo0 != null){
      ao4 = fo0.m4();
}
    ad4 = fd0 - fd1;
if(fo1 != null){
      ad1 = fo1.m3(ad2, ad3, ad4, fd0, fb0, fb1, fb0, fb1);
}
    Output.points[3][0] -= fd1;
    ad1 = ad2 - ad3;
    ad4 = fd0 - fd1;
if(ao2 != null){
      ao1 = ao2.m4(ao3, ao4, fo0, fo1);
}
    fb0 = ad1 > ad2;
    fb1 = ad3 < ad4;
if(ao1 != null){
      fd0 = ao1.m3(ao2, ao3, ao4, fo0, fd1, ad1, ad2, ad3, fb0, fb1, fb0, fb1);
}
if(fo1 != null){
      fb0 = fo1.m2(ao1, ao2, ao3, ao4, fb1, fb0, fb1, fb0);
}
    double ld0 = 440.922897337807;
if(fo0 != null){
      fo0.m2(ad3, ad4, fd0, fd1);
}
if(fo1 != null){
      ld0 = fo1.m3(ao1, ao2, ao3, ao4, ad1, ad2, ad3, ad4);
}
    fb1 = fb0 || fb1;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao1 != null){
      ao1.m1(ab1, ab2, ab3, ab4);
}
    fb0 = fb1 && ab1;
    fd1 = fd0 + fd1;
    ab2 = fd0 < fd1;
    boolean lb0 = true;
    ab2 = fd0 > fd1;
if(ao2 != null){
      fd0 = ao2.m3();
}
    ab3 = ab4 || fb0;
    fb1 = fd1 < fd0;
    lb0 = ab1 && ab2;
if(ao4 != null){
      ao3 = ao4.m4(fd1, fd0, fd1, fd0, ab3, ab4, fb0, fb1);
}
if(fo0 != null){
      lb0 = fo0.m2(fo1, ao1, ao2, ao3);
}
    double ld1 = 197.54176595086884;
    for(int i0=0; i0<10; i0++){
        fd0 = fd1 - ld1;
        ab1 = fd0 < fd1;
        for(int i1=0; i1<10; i1++){
if(fo0 != null){
              ao4 = fo0.m4(fo1, ao1, ao2, ao3, ld1, fd0, fd1, ld1, ab2, ab3, ab4, fb0);
}
            for(int i2=0; i2<10; i2++){
                for(int i3=0; i3<10; i3++){
if(ao4 != null){
                      fb1 = ao4.m2(fo0, fo1, ao1, ao2, lb0, ab1, ab2, ab3);
}
                    fd0 = fd1 - ld1;
                    }
                fd0 = fd1 + ld1;
}}}
Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Thought lo0 = Thought23.getInstance(ad2, ad3, ad4, fd0);
    boolean lb1 = false;
    lb1 = fd1 < ad1;
    Output.points[3][1] += ad2;
    Thought lo2 = Thought44.getInstance(ao1, ao2, ao3, ao4, ad3, ad4, fd0, fd1);
        double ld3 = 92.18241532104602;
    boolean lb4 = false;
if(fo0 != null){
      fo0.m1(lb4, ab1, ab2, ab3);
}
    ab4 = ld3 > ad1;
    ad2 = ad3 - ad4;
    boolean lb5 = false;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
boolean m2() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb0 = fd1 < fd0;
    double ld0 = 295.2990222053072;
if(fo0 != null){
      fo1 = fo0.m4();
}
    Output.points[3][2] -= fd0;
    fb1 = fd1 > ld0;
    double ld1 = 974.706281651004;
    fb0 = !fb1;
    ld1 *= -1;
    boolean lb2 = false;
    fd0 *= -1;
if(fo0 != null){
      fo1 = fo0.m4(fd1, ld0, ld1, fd0, lb2, fb0, fb1, lb2);
}
if(fo1 != null){
      fo1.m3(fo0, fo1, fo0, fo1);
}
    boolean lb3 = false;
    lb3 = fb0 && fb1;
if(fo0 != null){
      fo0.m1(fo1, fo0, fo1, fo0, fd1, ld0, ld1, fd0, lb2, lb3, fb0, fb1);
}
    fd1 = ld0 - ld1;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, lb2, lb3, fb0, fb1);
}
    lb2 = !lb3;

Thought.STACK_COUNTER++;
return fb0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ab2 = !ab3;
    double ld0 = 502.03684850330455;
    ab4 = fb0 || fb1;
    ab1 = !ab2;
    ld0 = fd0 + fd1;
if(fo1 != null){
      fo1.m2(ld0, fd0, fd1, ld0);
}
    ab3 = !ab4;
    Thought lo1 = Thought88.getInstance(fo0, fo1, fo0, fo1, fd0, fd1, ld0, fd0);
    boolean lb2 = true;
    ab4 = fb0 && fb1;
if(fo0 != null){
      fo0.m2(lb2, ab1, ab2, ab3);
}
if(fo1 != null){
      fo1.m1();
}
    ab4 = fb0 || fb1;
    boolean lb3 = false;
if(fo0 != null){
      lb2 = fo0.m2(fd1, ld0, fd0, fd1, lb3, ab1, ab2, ab3);
}
        Output.points[3][3] += ld0;
    Output.points[3][4] += fd0;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0);
}
    fd1 *= -1;
    Thought lo4 = Thought4.getInstance(fo1, fo0, fo1, fo0, ld0, fd0, fd1, ld0, ab4, fb0, fb1, lb2);
    boolean lb5 = true;
    fd0 *= -1;

Thought.STACK_COUNTER++;
return lb3;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, fb0, fb1, fb0, fb1);
}
    boolean lb0 = false;
if(fo0 != null){
      fo1 = fo0.m4(ad1, ad2, ad3, ad4);
}
    fd0 = fd1 + ad1;
    ad2 = ad3 + ad4;
    lb0 = fd0 > fd1;
    fb0 = ad1 < ad2;
    fb1 = lb0 && fb0;
    ad3 = ad4 + fd0;
if(fo1 != null){
      fb1 = fo1.m2(fo0, fo1, fo0, fo1, fd1, ad1, ad2, ad3);
}
    boolean lb1 = true;
    lb0 = !lb1;

Thought.STACK_COUNTER++;
return fb0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    boolean lb0 = true;
if(fo0 != null){
      ad2 = fo0.m3(ab1, ab2, ab3, ab4);
}
if(fo0 != null){
      fo1 = fo0.m4();
}
    Output.points[3][5] -= ad3;
    Thought lo1 = Thought65.getInstance(ad4, fd0, fd1, ad1, fb0, fb1, lb0, ab1);
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0);
}
    ad2 = ad3 - ad4;
if(fo1 != null){
      ab2 = fo1.m2(fo0, fo1, fo0, fo1, fd0, fd1, ad1, ad2, ab3, ab4, fb0, fb1);
}
    lb0 = ab1 && ab2;
    ad3 = ad4 + fd0;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, ab3, ab4, fb0, fb1);
}
if(fo1 != null){
      fo0 = fo1.m4(fd1, ad1, ad2, ad3);
}
    lb0 = ad4 < fd0;
    fd1 = ad1 + ad2;
    Thought lo2 = Thought14.getInstance(fo0, fo1, fo0, fo1, ad3, ad4, fd0, fd1);
    ab1 = ab2 && ab3;
    Thought lo3 = Thought7.getInstance(ab4, fb0, fb1, lb0);
    ab1 = ab2 || ab3;
if(fo1 != null){
      fo0 = fo1.m4();
}
    ad1 = ad2 - ad3;
if(fo1 != null){
      fo0 = fo1.m4(ad4, fd0, fd1, ad1, ab4, fb0, fb1, lb0);
}
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1);
}
if(fo0 != null){
      fo0.m1(fo1, fo0, fo1, fo0, ad2, ad3, ad4, fd0, ab1, ab2, ab3, ab4);
}

Thought.STACK_COUNTER++;
return fb0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(Thought ao1, Thought ao2, Thought ao3, Thought ao4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao2 != null){
      ao2.m1(ao3, ao4, fo0, fo1, fb0, fb1, fb0, fb1);
}
    fd1 *= -1;
    fb0 = !fb1;
    fd0 *= -1;
if(ao1 != null){
      fb0 = ao1.m2(fd1, fd0, fd1, fd0);
}
    fb1 = fd1 > fd0;
    fb0 = !fb1;
    for(int i0=0; i0<10; i0++){
        fd1 = fd0 - fd1;
        Thought lo0 = Thought38.getInstance(ao2, ao3, ao4, fo0, fd0, fd1, fd0, fd1);
if(fo1 != null){
          fo1.m2(fb0, fb1, fb0, fb1);
}
        double ld1 = 62.561464420330644;
        ld1 *= -1;
        boolean lb2 = true;
if(ao1 != null){
          lb2 = ao1.m2();
}
        }
if(ao2 != null){
          ao2.m3(fd1, fd0, fd1, fd0, fb1, fb0, fb1, fb0);
}
if(ao4 != null){
      ao3 = ao4.m4(fo0, fo1, ao1, ao2);
}
if(ao4 != null){
      ao3 = ao4.m4(fo0, fo1, ao1, ao2, fd1, fd0, fd1, fd0, fb1, fb0, fb1, fb0);
}
    fd1 *= -1;
    Output.points[3][6] += fd0;
if(ao4 != null){
      ao3 = ao4.m4(fo0, fo1, ao1, ao2, fb1, fb0, fb1, fb0);
}
    double ld3 = 538.7908498919184;
    Output.points[3][7] -= fd0;
    fb1 = fd1 > ld3;

Thought.STACK_COUNTER++;
return fb0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb1 = ad2 > ad3;
    boolean lb0 = true;
    ad4 *= -1;
    lb0 = !fb0;
    Output.points[3][8] -= fd0;
if(ao2 != null){
      ao1 = ao2.m4(fd1, ad1, ad2, ad3);
}
    boolean lb1 = true;
    fb0 = fb1 || lb0;
    double ld2 = 352.6432964801096;
    Output.points[4][0] += ad3;
    Thought lo3 = Thought98.getInstance(ao3, ao4, fo0, fo1, ad4, fd0, fd1, ld2);
    Output.points[4][1] += ad1;
    lb1 = fb0 || fb1;
if(ao1 != null){
      ao1.m3(lb0, lb1, fb0, fb1);
}
    Thought lo4 = Thought68.getInstance();

Thought.STACK_COUNTER++;
return lb0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao2 != null){
      fd0 = ao2.m3(fd1, fd0, fd1, fd0, ab2, ab3, ab4, fb0);
}
    fb1 = fd1 < fd0;
    double ld0 = 9.983431105713297;
    ab1 = ab2 && ab3;
    ab4 = fd0 > fd1;
    fb0 = ld0 < fd0;
    fd1 = ld0 + fd0;
    fb1 = fd1 < ld0;
    double ld1 = 537.8575081752269;
if(ao3 != null){
      ab1 = ao3.m2(ao4, fo0, fo1, ao1);
}
    double ld2 = 648.4196030565313;
if(ao2 != null){
      ab2 = ao2.m2(ao3, ao4, fo0, fo1, ld1, ld2, fd0, fd1, ab3, ab4, fb0, fb1);
}

Thought.STACK_COUNTER++;
return ab1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ad1 = ad2 - ad3;
    ab2 = ab3 && ab4;
    fb0 = fb1 || ab1;
        ab2 = ab3 || ab4;
    fb0 = ad4 < fd0;
    fd1 *= -1;
if(ao2 != null){
      ao1 = ao2.m4(ao3, ao4, fo0, fo1, fb1, ab1, ab2, ab3);
}
    ab4 = fb0 && fb1;
    ab1 = !ab2;
if(ao1 != null){
      ad1 = ao1.m3(ad2, ad3, ad4, fd0);
}
    Thought lo0 = Thought68.getInstance(ao2, ao3, ao4, fo0, fd1, ad1, ad2, ad3);
    boolean lb1 = false;
    Output.points[4][2] -= ad4;

Thought.STACK_COUNTER++;
return ab2;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
double m3() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    double ld0 = 970.9982935350486;
    for(int i0=0; i0<10; i0++){
        ld0 *= -1;
        fd0 *= -1;
        fd1 = ld0 - fd0;
        fd1 = ld0 - fd0;
        fb1 = fd1 > ld0;
        fb0 = fb1 && fb0;
if(fo1 != null){
          fd0 = fo1.m3(fb1, fb0, fb1, fb0);
}
        fd1 = ld0 - fd0;
        fb1 = fb0 && fb1;
        fb0 = fd1 < ld0;
        Thought lo1 = Thought89.getInstance();
        fb1 = fd0 < fd1;
if(fo1 != null){
          fo0 = fo1.m4(ld0, fd0, fd1, ld0, fb0, fb1, fb0, fb1);
}
        fb0 = fb1 && fb0;
if(fo0 != null){
          fo0.m2(fo1, fo0, fo1, fo0);
}
        fd0 *= -1;
        fb1 = fb0 && fb1;
        fd1 *= -1;
if(fo1 != null){
          fo1.m1(fo0, fo1, fo0, fo1, ld0, fd0, fd1, ld0, fb0, fb1, fb0, fb1);
}
        Output.points[4][3] += fd0;
        fd1 = ld0 + fd0;
        Output.points[4][4] += fd1;
}
Thought.STACK_COUNTER++;
return ld0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd1 = fd0 + fd1;
    ab1 = fd0 > fd1;
    Output.points[4][5] -= fd0;
    ab2 = fd1 > fd0;
        double ld0 = 338.1014452224743;
    fd0 = fd1 + ld0;
    ab3 = !ab4;
    boolean lb1 = false;
    Output.points[4][6] -= fd0;
    ab4 = !fb0;
    boolean lb2 = true;
    Output.points[4][7] -= fd1;
    Output.points[4][8] -= ld0;
    double ld3 = 228.18859586666704;
if(fo0 != null){
      fo0.m1(fo1, fo0, fo1, fo0, fb0, fb1, lb1, lb2);
}
    ld3 = fd0 + fd1;
    ab1 = ld0 > ld3;
    fd0 *= -1;

Thought.STACK_COUNTER++;
return fd1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb1 = fb0 && fb1;
    Output.points[5][0] += ad1;
    fb0 = fb1 && fb0;
if(fo0 != null){
      fo1 = fo0.m4(ad2, ad3, ad4, fd0);
}
if(fo1 != null){
      fo1.m1(fo0, fo1, fo0, fo1, fd1, ad1, ad2, ad3);
}
    fb1 = ad4 < fd0;
    fb0 = fd1 > ad1;
        fb1 = fb0 || fb1;
    ad2 *= -1;
    ad3 *= -1;
    boolean lb0 = false;
    Thought lo1 = Thought25.getInstance(lb0, fb0, fb1, lb0);
    ad4 = fd0 - fd1;
    double ld2 = 897.4155125469474;

Thought.STACK_COUNTER++;
return ld2;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(fo1 != null){
      fo0 = fo1.m4();
}
    boolean lb0 = true;
if(fo0 != null){
      fo0.m2(ad2, ad3, ad4, fd0, ab1, ab2, ab3, ab4);
}
    fd1 = ad1 + ad2;
    Output.points[5][1] += ad3;
    fb0 = fb1 && lb0;
if(fo1 != null){
      fo1.m1(fo0, fo1, fo0, fo1);
}
if(fo0 != null){
          ad4 = fo0.m3(fo1, fo0, fo1, fo0, fd0, fd1, ad1, ad2, ab1, ab2, ab3, ab4);
}
    Thought lo1 = Thought24.getInstance(fo1, fo0, fo1, fo0, fb0, fb1, lb0, ab1);
    ab2 = !ab3;
    boolean lb2 = false;
if(fo0 != null){
      fo1 = fo0.m4(ad3, ad4, fd0, fd1);
}
    Thought lo3 = Thought79.getInstance(fo1, fo0, fo1, fo0, ad1, ad2, ad3, ad4);
    Thought lo4 = Thought22.getInstance(ab3, ab4, fb0, fb1);
    Thought lo5 = Thought13.getInstance();
if(fo1 != null){
      lb0 = fo1.m2(fd0, fd1, ad1, ad2, lb2, ab1, ab2, ab3);
}
    boolean lb6 = false;
    ab3 = ad3 < ad4;

Thought.STACK_COUNTER++;
return fd0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(Thought ao1, Thought ao2, Thought ao3, Thought ao4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb0 = fb1 && fb0;
    fd1 *= -1;
if(ao1 != null){
      fb1 = ao1.m2(ao2, ao3, ao4, fo0);
}
if(ao1 != null){
      fo1 = ao1.m4(ao2, ao3, ao4, fo0, fd0, fd1, fd0, fd1, fb0, fb1, fb0, fb1);
}
    fb0 = fd0 > fd1;
    fd0 *= -1;
if(ao1 != null){
      fo1 = ao1.m4(ao2, ao3, ao4, fo0, fb1, fb0, fb1, fb0);
}
        Output.points[5][2] += fd1;
    if (fb1) {
        fb0 = fd0 > fd1;
        fd0 *= -1;
        Output.points[5][3] += fd1;
        fb1 = fb0 && fb1;
if(fo1 != null){
          fd0 = fo1.m3(fd1, fd0, fd1, fd0);
}
if(ao1 != null){
          fd1 = ao1.m3(ao2, ao3, ao4, fo0, fd0, fd1, fd0, fd1);
}
        fb0 = fd0 > fd1;
        fb1 = fb0 && fb1;
        boolean lb0 = true;
        fd0 *= -1;
        double ld1 = 988.6958025323563;
if(ao1 != null){
          fo1 = ao1.m4(lb0, fb0, fb1, lb0);
}
        fb0 = !fb1;
        Output.points[5][4] += fd0;
if(ao2 != null){
          fd1 = ao2.m3();
}
}
Thought.STACK_COUNTER++;
return fd0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb0 = ad2 < ad3;
    double ld0 = 575.2517432738949;
if(ao2 != null){
      ao1 = ao2.m4(ad3, ad4, fd0, fd1, fb1, fb0, fb1, fb0);
}
    ld0 = ad1 - ad2;
    fb1 = ad3 < ad4;
if(ao4 != null){
      ao3 = ao4.m4(fo0, fo1, ao1, ao2);
}
    fb0 = fd0 > fd1;
    double ld1 = 408.287744124202;
    fb1 = !fb0;
    fb1 = fb0 && fb1;
if(ao3 != null){
      ld0 = ao3.m3(ao4, fo0, fo1, ao1, ld1, ad1, ad2, ad3, fb0, fb1, fb0, fb1);
}
    fb0 = ad4 < fd0;
    fb1 = fb0 || fb1;
    fd1 = ld0 + ld1;

Thought.STACK_COUNTER++;
return ad1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao2 != null){
      ab1 = ao2.m2(ao3, ao4, fo0, fo1, ab2, ab3, ab4, fb0);
}
    fb1 = fd1 > fd0;
if(ao2 != null){
      ao1 = ao2.m4(fd1, fd0, fd1, fd0);
}
    boolean lb0 = false;
    lb0 = ab1 || ab2;
    ab3 = !ab4;
    fb0 = fb1 && lb0;
    Output.points[5][5] -= fd1;
if(ao4 != null){
      ao3 = ao4.m4(fo0, fo1, ao1, ao2, fd0, fd1, fd0, fd1);
}
    Thought lo1 = Thought10.getInstance(ab1, ab2, ab3, ab4);
    for(int i0=0; i0<10; i0++){
        boolean lb2 = false;
        Output.points[5][6] += fd0;
        for(int i1=0; i1<10; i1++){
            ab4 = fb0 && fb1;
            }
        lb2 = !lb0;
if(ao4 != null){
          ao3 = ao4.m4();
}
        ab1 = fd1 < fd0;
        boolean lb3 = false;
        boolean lb4 = false;
if(fo1 != null){
          fo0 = fo1.m4(fd1, fd0, fd1, fd0, lb0, ab1, ab2, ab3);
}
}
Thought.STACK_COUNTER++;
return fd1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ab1 = ab2 && ab3;
if(ao1 != null){
      ad1 = ao1.m3(ao2, ao3, ao4, fo0);
}
if(fo1 != null){
      ad2 = fo1.m3(ao1, ao2, ao3, ao4, ad3, ad4, fd0, fd1, ab4, fb0, fb1, ab1);
}
        ad1 = ad2 + ad3;
    ab2 = ab3 && ab4;
    fb0 = !fb1;
if(fo0 != null){
      ab1 = fo0.m2(fo1, ao1, ao2, ao3, ab2, ab3, ab4, fb0);
}
if(ao4 != null){
      ad4 = ao4.m3(fd0, fd1, ad1, ad2);
}
    fb1 = ab1 || ab2;
if(fo1 != null){
          fo0 = fo1.m4(ao1, ao2, ao3, ao4, ad3, ad4, fd0, fd1);
}
if(fo0 != null){
      fo0.m1(ab3, ab4, fb0, fb1);
}
if(fo1 != null){
      fo1.m3();
}
if(ao2 != null){
      ao1 = ao2.m4(ad1, ad2, ad3, ad4, ab1, ab2, ab3, ab4);
}
    double ld0 = 404.09407783876554;
    boolean lb1 = false;

Thought.STACK_COUNTER++;
return ad4;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
Thought m4() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb0 = fb1 && fb0;
    Thought lo0 = Thought81.getInstance(fo0, fo1, fo0, fo1);
    fb1 = !fb0;
if(fo0 != null){
      fo0.m3(fo1, fo0, fo1, fo0, fd1, fd0, fd1, fd0, fb1, fb0, fb1, fb0);
}
    fb1 = fd1 > fd0;
    boolean lb1 = true;
if(fo1 != null){
      fo1.m3(fo0, fo1, fo0, fo1, lb1, fb0, fb1, lb1);
}
    fb0 = fb1 && lb1;
    Thought lo2 = Thought29.getInstance(fd1, fd0, fd1, fd0);
if(fo0 != null){
      fd1 = fo0.m3(fo1, fo0, fo1, fo0, fd0, fd1, fd0, fd1);
}
    fd0 = fd1 - fd0;
    fd1 = fd0 + fd1;
if(fo1 != null){
      fo1.m3(fb0, fb1, lb1, fb0);
}
    fd0 = fd1 - fd0;
    fb1 = lb1 || fb0;
    double ld3 = 546.9164862189033;
if(fo0 != null){
      fd0 = fo0.m3();
}
    fb1 = lb1 && fb0;
    Thought lo4 = Thought4.getInstance(fd1, ld3, fd0, fd1, fb1, lb1, fb0, fb1);
    Thought lo5 = Thought69.getInstance(fo1, fo0, fo1, fo0);
    double ld6 = 210.9869210636615;
if(fo1 != null){
      lb1 = fo1.m2(fo0, fo1, fo0, fo1, ld3, ld6, fd0, fd1, fb0, fb1, lb1, fb0);
}
    fb1 = ld3 < ld6;

Thought.STACK_COUNTER++;
return fo0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Thought lo0 = Thought58.getInstance(fo1, fo0, fo1, fo0, ab1, ab2, ab3, ab4);
if(fo0 != null){
      fo1 = fo0.m4(fd0, fd1, fd0, fd1);
}
if(fo1 != null){
      fb0 = fo1.m2(fo0, fo1, fo0, fo1, fd0, fd1, fd0, fd1);
}
    Thought lo1 = Thought51.getInstance(fb1, ab1, ab2, ab3);
if(fo1 != null){
      fo0 = fo1.m4();
}
    fd0 *= -1;
    Thought lo2 = Thought41.getInstance(fd1, fd0, fd1, fd0, ab4, fb0, fb1, ab1);
    fd1 = fd0 - fd1;
    fd0 = fd1 + fd0;
    ab2 = ab3 || ab4;
    Output.points[5][7] += fd1;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1);
}
    fb0 = fd0 < fd1;

Thought.STACK_COUNTER++;
return fo0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb1 = !fb0;
    Thought lo0 = Thought35.getInstance(fo1, fo0, fo1, fo0, ad1, ad2, ad3, ad4, fb1, fb0, fb1, fb0);
    fb1 = fd0 > fd1;
    ad1 *= -1;
    Output.points[5][8] += ad2;
    double ld1 = 233.84949730169873;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, fb0, fb1, fb0, fb1);
}
    fb0 = ad2 < ad3;
    fb1 = !fb0;
    ad4 = fd0 - fd1;
    fb1 = fb0 || fb1;

Thought.STACK_COUNTER++;
return fo1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ad1 *= -1;
    Thought lo0 = Thought20.getInstance(ad2, ad3, ad4, fd0);
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, fd1, ad1, ad2, ad3);
}
    ab1 = ad4 > fd0;
    ab2 = !ab3;
    fd1 *= -1;
    double ld1 = 240.08362664498216;
if(fo0 != null){
      ld1 = fo0.m3(ab4, fb0, fb1, ab1);
}
    ab2 = ad1 > ad2;
if(fo0 != null){
      fo1 = fo0.m4();
}
    ad3 = ad4 - fd0;
    ab3 = fd1 > ld1;
if(fo1 != null){
      ab4 = fo1.m2(ad1, ad2, ad3, ad4, fb0, fb1, ab1, ab2);
}
    boolean lb2 = false;
    double ld3 = 394.2282537733003;
    ab2 = ad4 > fd0;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1);
}
if(fo0 != null){
      ab3 = fo0.m2(fo1, fo0, fo1, fo0, fd1, ld1, ld3, ad1, ab4, fb0, fb1, lb2);
}
    double ld4 = 65.79625741870306;
    if (ab1) {
        boolean lb5 = false;
        ad1 *= -1;
        ab1 = ab2 || ab3;
        ab4 = !fb0;
        double ld6 = 132.40083484319723;
        fb1 = lb5 || lb2;
        double ld7 = 170.48264643231155;
}
Thought.STACK_COUNTER++;
return fo1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(Thought ao1, Thought ao2, Thought ao3, Thought ao4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd0 *= -1;
    fb0 = !fb1;
if(ao1 != null){
      fd1 = ao1.m3(ao2, ao3, ao4, fo0, fb0, fb1, fb0, fb1);
}
    fb0 = fd0 > fd1;
if(fo1 != null){
      fb1 = fo1.m2(fd0, fd1, fd0, fd1);
}
    fb0 = fd0 < fd1;
    fb1 = fd0 > fd1;
if(ao1 != null){
      fd0 = ao1.m3(ao2, ao3, ao4, fo0, fd1, fd0, fd1, fd0);
}
    fd1 *= -1;
    if (fb0) {
        fd0 *= -1;
}
Thought.STACK_COUNTER++;
return fo1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Thought lo0 = Thought47.getInstance(fb1, fb0, fb1, fb0);
    boolean lb1 = false;
    fb0 = !fb1;
    Output.points[6][0] += ad2;
    Thought lo2 = Thought16.getInstance();
    ad3 = ad4 - fd0;
if(ao2 != null){
      ao1 = ao2.m4(fd1, ad1, ad2, ad3, lb1, fb0, fb1, lb1);
}
    boolean lb3 = false;
    ad4 *= -1;
if(ao3 != null){
      ao3.m1(ao4, fo0, fo1, ao1);
}
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, fd0, fd1, ad1, ad2, lb3, fb0, fb1, lb1);
}
    ad3 = ad4 + fd0;
    fd1 = ad1 + ad2;
    Thought lo4 = Thought61.getInstance(ao2, ao3, ao4, fo0, lb3, fb0, fb1, lb1);
    boolean lb5 = true;
if(fo1 != null){
      fo1.m1(ad3, ad4, fd0, fd1);
}
    Output.points[6][1] -= ad1;
if(ao1 != null){
      lb3 = ao1.m2(ao2, ao3, ao4, fo0, ad2, ad3, ad4, fd0);
}
if(ao1 != null){
          fo1 = ao1.m4(lb5, fb0, fb1, lb1);
}
    boolean lb6 = true;
    Thought lo7 = Thought55.getInstance();
    lb3 = !lb5;
    Output.points[6][2] += fd1;
if(ao2 != null){
      lb6 = ao2.m2(ad1, ad2, ad3, ad4, fb0, fb1, lb1, lb3);
}

Thought.STACK_COUNTER++;
return ao3;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd0 = fd1 + fd0;
    Output.points[6][3] -= fd1;
    boolean lb0 = false;
    fd0 = fd1 - fd0;
    lb0 = ab1 && ab2;
    ab3 = !ab4;
    fb0 = !fb1;
    lb0 = !ab1;
    boolean lb1 = false;
    ab1 = ab2 || ab3;
    boolean lb2 = true;
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1);
}
    ab3 = ab4 && fb0;
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, fd1, fd0, fd1, fd0, fb1, lb0, lb1, lb2);
}
    ab1 = fd1 > fd0;
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, ab2, ab3, ab4, fb0);
}
    fd1 *= -1;
    Thought lo3 = Thought38.getInstance(fd0, fd1, fd0, fd1);
    boolean lb4 = true;
if(ao2 != null){
      ao2.m2(ao3, ao4, fo0, fo1, fd0, fd1, fd0, fd1);
}
    fb0 = !fb1;
        fd0 *= -1;
if(ao2 != null){
      ao1 = ao2.m4(lb0, lb1, lb2, lb4);
}
if(ao4 != null){
      ao3 = ao4.m4();
}
    Thought lo5 = Thought11.getInstance(fd1, fd0, fd1, fd0, ab1, ab2, ab3, ab4);
    fb0 = fb1 || lb0;
if(fo1 != null){
      fo0 = fo1.m4(ao1, ao2, ao3, ao4);
}

Thought.STACK_COUNTER++;
return fo0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, ad2, ad3, ad4, fd0, ab2, ab3, ab4, fb0);
}
    fd1 = ad1 + ad2;
    ad3 *= -1;
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, fb1, ab1, ab2, ab3);
}
if(ao2 != null){
      ao2.m2(ad4, fd0, fd1, ad1);
}
    ad2 = ad3 + ad4;
    boolean lb0 = false;
    for(int i0=0; i0<10; i0++){
if(ao4 != null){
          ao3 = ao4.m4(fo0, fo1, ao1, ao2, fd0, fd1, ad1, ad2);
}
        ad3 *= -1;
        Thought lo1 = Thought81.getInstance(ab3, ab4, fb0, fb1);
        Output.points[6][4] += ad4;
        Thought lo2 = Thought84.getInstance();
        lb0 = ab1 || ab2;
        ab3 = fd0 < fd1;
if(ao4 != null){
          ao3 = ao4.m4(ad1, ad2, ad3, ad4, ab4, fb0, fb1, lb0);
}
        ab1 = fd0 < fd1;
if(fo1 != null){
          fo0 = fo1.m4(ao1, ao2, ao3, ao4);
}
        ad1 = ad2 - ad3;
if(fo0 != null){
          ab2 = fo0.m2(fo1, ao1, ao2, ao3, ad4, fd0, fd1, ad1, ab3, ab4, fb0, fb1);
}
        boolean lb3 = false;
if(fo0 != null){
          ao4 = fo0.m4(fo1, ao1, ao2, ao3, lb3, lb0, ab1, ab2);
}
        ab3 = ad2 > ad3;
        double ld4 = 314.5395812138287;
        double ld5 = 707.317681175908;
        ad2 = ad3 + ad4;
if(fo0 != null){
          ao4 = fo0.m4(fd0, fd1, ld4, ld5);
}
        ad1 *= -1;
        ad2 = ad3 - ad4;
}
Thought.STACK_COUNTER++;
return fo1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
void accept(String color) throws CountDownExc {
switch (color) {
case "e":
empty();
break;
case "own":
own();
break;
case "other":
other();
break;
default:
throw new RuntimeException("Illegal color!!!");
}
}
void empty() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(fo0 != null){
      fd0 = fo0.m3(fo1, fo0, fo1, fo0, fd1, fd0, fd1, fd0);
}
    fd1 *= -1;
if(fo0 != null){
      fo1 = fo0.m4(fb1, fb0, fb1, fb0);
}
    Thought lo0 = Thought27.getInstance();
if(fo0 != null){
      fo1 = fo0.m4(fd0, fd1, fd0, fd1, fb1, fb0, fb1, fb0);
}
    fb1 = fd0 < fd1;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0);
}
if(fo1 != null){
      fo1.m3(fo0, fo1, fo0, fo1, fd0, fd1, fd0, fd1, fb0, fb1, fb0, fb1);
}
        fb0 = !fb1;
    fd0 *= -1;
    fb0 = fd1 < fd0;
    fd1 = fd0 + fd1;
if(fo0 != null){
      fb1 = fo0.m2(fo1, fo0, fo1, fo0, fb0, fb1, fb0, fb1);
}
    fb0 = fd0 < fd1;
    Thought lo1 = Thought69.getInstance(fd0, fd1, fd0, fd1);
    fd0 *= -1;
    double ld2 = 725.0099514419227;
    fb1 = !fb0;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void own() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd1 *= -1;
        fd0 *= -1;
    fb1 = fb0 || fb1;
    fb0 = fb1 || fb0;
    fb1 = fd1 < fd0;
if(fo1 != null){
      fb0 = fo1.m2(fo0, fo1, fo0, fo1, fd1, fd0, fd1, fd0);
}
    fb1 = fd1 > fd0;
    fd1 = fd0 - fd1;
if(fo0 != null){
      fb0 = fo0.m2(fb1, fb0, fb1, fb0);
}
if(fo1 != null){
      fo1.m2();
}
    fb1 = fb0 && fb1;
    fb0 = !fb1;
if(fo1 != null){
      fo0 = fo1.m4(fd0, fd1, fd0, fd1, fb0, fb1, fb0, fb1);
}
    Output.points[6][5] += fd0;
if(fo0 != null){
      fd1 = fo0.m3(fo1, fo0, fo1, fo0);
}
    boolean lb0 = true;
if(fo1 != null){
      fo1.m2(fo0, fo1, fo0, fo1, fd0, fd1, fd0, fd1, lb0, fb0, fb1, lb0);
}
    Thought lo1 = Thought10.getInstance(fo0, fo1, fo0, fo1, fb0, fb1, lb0, fb0);
    double ld2 = 292.1085250458751;
    fb1 = !lb0;
    fb0 = !fb1;
    double ld3 = 46.42425091948069;
    Output.points[6][6] += ld2;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void other() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd1 *= -1;
    fb0 = fd0 > fd1;
    fb1 = fb0 && fb1;
    boolean lb0 = true;
    boolean lb1 = false;
    double ld2 = 82.65073686599996;
    Thought lo3 = Thought68.getInstance(ld2, fd0, fd1, ld2);
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, fd0, fd1, ld2, fd0);
}
if(fo1 != null){
      fo0 = fo1.m4(lb0, lb1, fb0, fb1);
}
    double ld4 = 977.5625266414376;
    lb0 = lb1 || fb0;
    fb1 = fd0 < fd1;
    Thought lo5 = Thought97.getInstance();
if(fo1 != null){
      fo0 = fo1.m4(ld2, ld4, fd0, fd1, lb0, lb1, fb0, fb1);
}
if(fo1 != null){
          fo0 = fo1.m4(fo0, fo1, fo0, fo1);
}
if(fo0 != null){
      fo0.m2(fo1, fo0, fo1, fo0, ld2, ld4, fd0, fd1, lb0, lb1, fb0, fb1);
}
    double ld6 = 699.4558030008075;
    ld2 *= -1;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, lb0, lb1, fb0, fb1);
}
if(fo1 != null){
      fo1.m3(ld4, ld6, fd0, fd1);
}
    ld2 *= -1;
    double ld7 = 419.7604347923332;
if(fo0 != null){
      fo0.m3(fo1, fo0, fo1, fo0, ld4, ld6, ld7, fd0);
}
    fd1 = ld2 - ld4;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
}
