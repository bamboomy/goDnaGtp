package genetic;
import java.util.ArrayList;
class Thought80 extends Thought{
private static ArrayList<Thought80> instances = new ArrayList<>();
private static int instanceCounter = 0;
private boolean fb0 = false;
private boolean fb1 = false;
private double fd0 = 290.2561945244121;
private double fd1 = 773.5659306940123;
private Thought fo0 = null;
private Thought fo1 = null;
Thought80 (){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
Thought.STACK_COUNTER++;
}
Thought80 (boolean ab1, boolean ab2, boolean ab3, boolean ab4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fb0 = ab1; 
fb1 = ab2; 
Thought.STACK_COUNTER++;
}
Thought80 (double ad1, double ad2, double ad3, double ad4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fd0 = ad1; 
fd1 = ad2; 
Thought.STACK_COUNTER++;
}
Thought80 (double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fb0 = ab1; 
fb1 = ab2; 
fd0 = ad1; 
fd1 = ad2; 
Thought.STACK_COUNTER++;
}
Thought80 (Thought ao1, Thought ao2, Thought ao3, Thought ao4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fo0 = ao1; 
fo1 = ao2; 
Thought.STACK_COUNTER++;
}
Thought80 (Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fd0 = ad1; 
fd1 = ad2; 
fo0 = ao1; 
fo1 = ao2; 
Thought.STACK_COUNTER++;
}
Thought80 (Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fb0 = ab1; 
fb1 = ab2; 
fo0 = ao1; 
fo1 = ao2; 
Thought.STACK_COUNTER++;
}
Thought80 (Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fb0 = ab1; 
fb1 = ab2; 
fd0 = ad1; 
fd1 = ad2; 
fo0 = ao1; 
fo1 = ao2; 
Thought.STACK_COUNTER++;
}
public static Thought80 getInstance(){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought80 instance = new Thought80 (); 
instances.add(instance); 
return instance; 
} 
}
public static Thought80 getInstance(boolean ab1, boolean ab2, boolean ab3, boolean ab4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought80 instance = new Thought80 ( ab1,  ab2,  ab3,  ab4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought80 getInstance(double ad1, double ad2, double ad3, double ad4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought80 instance = new Thought80 ( ad1,  ad2,  ad3,  ad4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought80 getInstance(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought80 instance = new Thought80 ( ad1,  ad2,  ad3,  ad4,  ab1,  ab2,  ab3,  ab4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought80 getInstance(Thought ao1, Thought ao2, Thought ao3, Thought ao4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought80 instance = new Thought80 ( ao1,  ao2,  ao3,  ao4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought80 getInstance(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought80 instance = new Thought80 ( ao1,  ao2,  ao3,  ao4,  ad1,  ad2,  ad3,  ad4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought80 getInstance(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought80 instance = new Thought80 ( ao1,  ao2,  ao3,  ao4,  ab1,  ab2,  ab3,  ab4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought80 getInstance(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought80 instance = new Thought80 ( ao1,  ao2,  ao3,  ao4,  ad1,  ad2,  ad3,  ad4,  ab1,  ab2,  ab3,  ab4); 
instances.add(instance); 
return instance; 
} 
}
void m1() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb0 = fd1 > fd0;
    fd1 *= -1;
    fd0 = fd1 - fd0;
    fb1 = fd1 < fd0;
if(fo0 != null){
      fd1 = fo0.m3(fd0, fd1, fd0, fd1);
}
    Thought lo0 = Thought373.getInstance(fo1, fo0, fo1, fo0, fd0, fd1, fd0, fd1);
if(fo1 != null){
      fd0 = fo1.m3(fd1, fd0, fd1, fd0, fb0, fb1, fb0, fb1);
}
    double ld1 = 270.06446644785984;
    fb0 = !fb1;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, fb0, fb1, fb0, fb1);
}
    boolean lb2 = true;
    Output.points[1][7] -= fd0;
    fd1 = ld1 + fd0;
    boolean lb3 = false;
if(fo0 != null){
      fo0.m1();
}
if(fo1 != null){
      fd1 = fo1.m3(fo0, fo1, fo0, fo1, ld1, fd0, fd1, ld1, lb2, lb3, fb0, fb1);
}
    fd0 = fd1 - ld1;
    lb2 = fd0 > fd1;
if(fo0 != null){
      fo0.m3(lb3, fb0, fb1, lb2);
}
    ld1 = fd0 - fd1;
    lb3 = fb0 && fb1;
if(fo1 != null){
      ld1 = fo1.m3(fo0, fo1, fo0, fo1);
}
if(fo0 != null){
      fo0.m1(fd0, fd1, ld1, fd0);
}
    fd1 = ld1 + fd0;
    fd1 = ld1 + fd0;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, fd0, fd1, fd0, fd1);
}
if(fo0 != null){
      fo1 = fo0.m4(fd0, fd1, fd0, fd1, ab1, ab2, ab3, ab4);
}
    fb0 = fb1 || ab1;
    Output.points[1][8] += fd0;
    double ld0 = 393.75664538482624;
    Thought lo1 = Thought165.getInstance(fo1, fo0, fo1, fo0, ab2, ab3, ab4, fb0);
if(fo1 != null){
      fo1.m1();
}
    fb1 = ab1 || ab2;
    boolean lb2 = true;
    ab2 = ab3 && ab4;
    fd0 = fd1 + ld0;
    fb0 = fd0 < fd1;
    ld0 = fd0 - fd1;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, ld0, fd0, fd1, ld0, fb1, lb2, ab1, ab2);
}
    fd0 = fd1 - ld0;
    Thought lo3 = Thought227.getInstance(ab3, ab4, fb0, fb1);
    lb2 = fd0 > fd1;
    ld0 = fd0 + fd1;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1);
}
    ld0 *= -1;
    ab1 = ab2 && ab3;
    ab4 = fd0 > fd1;
    fb0 = ld0 > fd0;
    fb1 = fd1 < ld0;
    fd0 = fd1 - ld0;
if(fo0 != null){
      lb2 = fo0.m2(fd0, fd1, ld0, fd0);
}
    ab1 = fd1 > ld0;
    boolean lb4 = false;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, fd0, fd1, ld0, fd0);
}

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(fo0 != null){
      fo1 = fo0.m4(ad1, ad2, ad3, ad4, fb0, fb1, fb0, fb1);
}
if(fo1 != null){
      fd0 = fo1.m3(fo0, fo1, fo0, fo1, fb0, fb1, fb0, fb1);
}
if(fo1 != null){
      fo0 = fo1.m4();
}
    fb0 = fd1 < ad1;
    ad2 = ad3 - ad4;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, fd0, fd1, ad1, ad2, fb1, fb0, fb1, fb0);
}
if(fo0 != null){
      fo0.m2(fb1, fb0, fb1, fb0);
}
    Output.points[2][0] += ad3;
if(fo1 != null){
      fo1.m3(fo0, fo1, fo0, fo1);
}
    double ld0 = 495.5858009612373;
    ad3 = ad4 - fd0;
    boolean lb1 = false;
if(fo0 != null){
          fo0.m2(fd1, ld0, ad1, ad2);
}
    Output.points[2][1] -= ad3;
    fb0 = !fb1;
    lb1 = ad4 > fd0;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, fd1, ld0, ad1, ad2);
}
    ad3 *= -1;
if(fo0 != null){
      fo1 = fo0.m4(ad4, fd0, fd1, ld0, fb0, fb1, lb1, fb0);
}
    for(int i0=0; i0<10; i0++){
        ad1 = ad2 + ad3;
        fb1 = ad4 > fd0;
        for(int i1=0; i1<10; i1++){
if(fo1 != null){
              fd1 = fo1.m3(fo0, fo1, fo0, fo1, lb1, fb0, fb1, lb1);
}
            fb0 = ld0 > ad1;
}}
Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    if (ab1) {
        ad1 *= -1;
        ab2 = ad2 > ad3;
        double ld0 = 720.7462817787455;
        ab3 = ad3 > ad4;
        } else {
        ab4 = fd1 > ad1;
        fb0 = fb1 || ab1;
if(fo0 != null){
          fo0.m1();
}
        for(int i0=0; i0<10; i0++){
            Thought lo1 = Thought313.getInstance(fo1, fo0, fo1, fo0, ad2, ad3, ad4, fd0, ab2, ab3, ab4, fb0);
            boolean lb2 = true;
            Thought lo3 = Thought27.getInstance(fb0, fb1, lb2, ab1);
if(fo1 != null){
              fo1.m2(fo0, fo1, fo0, fo1);
}
            fd1 = ad1 - ad2;
            Thought lo4 = Thought30.getInstance(ad3, ad4, fd0, fd1);
            boolean lb5 = true;
if(fo1 != null){
              fo0 = fo1.m4(fo0, fo1, fo0, fo1, ad1, ad2, ad3, ad4);
}
            if (ab1) {
                boolean lb6 = false;
                double ld7 = 424.547702643597;
                ad4 = fd0 + fd1;
                double ld8 = 959.3708404729433;
if(fo1 != null){
                  fo0 = fo1.m4(ld7, ld8, ad1, ad2, ab1, ab2, ab3, ab4);
}
}}}
Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(Thought ao1, Thought ao2, Thought ao3, Thought ao4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao2 != null){
      ao1 = ao2.m4(ao3, ao4, fo0, fo1, fb1, fb0, fb1, fb0);
}
    fb1 = fb0 || fb1;
if(ao1 != null){
      fd0 = ao1.m3();
}
    Output.points[2][2] -= fd1;
    fb0 = fd0 > fd1;
    fb1 = fd0 > fd1;
    fd0 *= -1;
    Thought lo0 = Thought48.getInstance(ao2, ao3, ao4, fo0, fd1, fd0, fd1, fd0, fb0, fb1, fb0, fb1);
    fd1 *= -1;
    Thought lo1 = Thought85.getInstance(fb0, fb1, fb0, fb1);
if(fo1 != null){
      fb0 = fo1.m2(ao1, ao2, ao3, ao4);
}
    fd0 *= -1;
    double ld2 = 173.36359885063231;
    fd0 = fd1 + ld2;
    fb1 = fb0 || fb1;
    boolean lb3 = true;
    boolean lb4 = true;
        boolean lb5 = true;
    boolean lb6 = false;
    Thought lo7 = Thought266.getInstance(fd0, fd1, ld2, fd0);

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    double ld0 = 798.644487537762;
    boolean lb1 = true;
    ld0 *= -1;
    if (lb1) {
        boolean lb2 = false;
if(ao1 != null){
          ad1 = ao1.m3(ao2, ao3, ao4, fo0, ad2, ad3, ad4, fd0);
}
        Thought lo3 = Thought227.getInstance(fd1, ld0, ad1, ad2, lb1, fb0, fb1, lb2);
        Output.points[2][3] -= ad3;
        lb1 = fb0 || fb1;
        lb2 = lb1 || fb0;
        for(int i0=0; i0<10; i0++){
            fb1 = !lb2;
if(ao1 != null){
              fo1 = ao1.m4(ao2, ao3, ao4, fo0, lb1, fb0, fb1, lb2);
}
if(ao1 != null){
              fo1 = ao1.m4();
}
if(ao2 != null){
              ad4 = ao2.m3(ao3, ao4, fo0, fo1, fd0, fd1, ld0, ad1, lb1, fb0, fb1, lb2);
}
            ad2 = ad3 + ad4;
            fd0 = fd1 - ld0;
            lb1 = ad1 < ad2;
if(ao2 != null){
              ao1 = ao2.m4(fb0, fb1, lb2, lb1);
}
            fb0 = fb1 || lb2;
if(ao3 != null){
              lb1 = ao3.m2(ao4, fo0, fo1, ao1);
}
if(ao2 != null){
              ao2.m1(ad3, ad4, fd0, fd1);
}
if(ao3 != null){
              ao3.m2(ao4, fo0, fo1, ao1, ld0, ad1, ad2, ad3);
}
            boolean lb4 = false;
}}
Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Thought lo0 = Thought114.getInstance(fd0, fd1, fd0, fd1, ab1, ab2, ab3, ab4);
if(ao2 != null){
      fd0 = ao2.m3(ao3, ao4, fo0, fo1, fb0, fb1, ab1, ab2);
}
    if (ab3) {
if(ao1 != null){
          ab4 = ao1.m2();
}
        fb0 = fd1 > fd0;
        } else if (fb1) {
if(ao2 != null){
          fd1 = ao2.m3(ao3, ao4, fo0, fo1, fd0, fd1, fd0, fd1, ab1, ab2, ab3, ab4);
}
        fb0 = fd0 > fd1;
        Output.points[2][4] += fd0;
if(ao1 != null){
          fb1 = ao1.m2(ab1, ab2, ab3, ab4);
}
        fb0 = !fb1;
        ab1 = !ab2;
        Thought lo1 = Thought307.getInstance(ao2, ao3, ao4, fo0);
}
Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao3 != null){
      ao2 = ao3.m4(ad2, ad3, ad4, fd0);
}
if(ao4 != null){
      ao4.m2(fo0, fo1, ao1, ao2, fd1, ad1, ad2, ad3);
}
    ab1 = !ab2;
if(ao3 != null){
      ao3.m2(ad4, fd0, fd1, ad1, ab3, ab4, fb0, fb1);
}
    boolean lb0 = true;
    ad2 = ad3 - ad4;
    fd0 = fd1 - ad1;
if(fo0 != null){
      ao4 = fo0.m4(fo1, ao1, ao2, ao3, lb0, ab1, ab2, ab3);
}
        boolean lb1 = false;
    Output.points[2][5] -= ad2;
if(ao4 != null){
      ao4.m3();
}
    ab3 = ad3 > ad4;
if(fo0 != null){
      fo0.m1(fo1, ao1, ao2, ao3, fd0, fd1, ad1, ad2, ab4, fb0, fb1, lb0);
}
if(fo0 != null){
      ao4 = fo0.m4(lb1, ab1, ab2, ab3);
}
    boolean lb2 = true;
    ab3 = ad3 > ad4;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
boolean m2() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Output.points[2][6] += fd0;
    Output.points[2][7] += fd1;
    fb0 = fb1 && fb0;
if(fo1 != null){
      fo1.m2(fo0, fo1, fo0, fo1);
}
    fd0 *= -1;
if(fo0 != null){
      fd1 = fo0.m3(fd0, fd1, fd0, fd1);
}
if(fo1 != null){
      fd0 = fo1.m3(fo0, fo1, fo0, fo1, fd1, fd0, fd1, fd0);
}
    Thought lo0 = Thought371.getInstance(fd1, fd0, fd1, fd0, fb1, fb0, fb1, fb0);
if(fo0 != null){
      fd1 = fo0.m3(fo1, fo0, fo1, fo0, fb1, fb0, fb1, fb0);
}
    fb1 = !fb0;
    fb1 = fb0 && fb1;
if(fo1 != null){
      fo1.m1();
}
    fb0 = fb1 && fb0;
if(fo0 != null){
      fd0 = fo0.m3(fo1, fo0, fo1, fo0, fd1, fd0, fd1, fd0, fb1, fb0, fb1, fb0);
}
if(fo1 != null){
      fb1 = fo1.m2(fb0, fb1, fb0, fb1);
}

Thought.STACK_COUNTER++;
return fb0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ab2 = !ab3;
    Thought lo0 = Thought47.getInstance(fo0, fo1, fo0, fo1);
if(fo0 != null){
      ab4 = fo0.m2(fd1, fd0, fd1, fd0);
}
    fd1 *= -1;
if(fo1 != null){
      fd0 = fo1.m3(fo0, fo1, fo0, fo1, fd1, fd0, fd1, fd0);
}
if(fo0 != null){
      fd1 = fo0.m3(fd0, fd1, fd0, fd1, fb0, fb1, ab1, ab2);
}
if(fo1 != null){
      fo1.m1(fo0, fo1, fo0, fo1, ab3, ab4, fb0, fb1);
}
if(fo1 != null){
      fo0 = fo1.m4();
}
    ab1 = fd0 < fd1;
    fd0 = fd1 + fd0;
if(fo0 != null){
      fd1 = fo0.m3(fo1, fo0, fo1, fo0, fd0, fd1, fd0, fd1, ab2, ab3, ab4, fb0);
}
if(fo1 != null){
      fd0 = fo1.m3(fb1, ab1, ab2, ab3);
}
    ab4 = fd1 > fd0;
    fb0 = fd1 > fd0;
if(fo0 != null){
      fd1 = fo0.m3(fo1, fo0, fo1, fo0);
}
if(fo1 != null){
      fb1 = fo1.m2(fd0, fd1, fd0, fd1);
}
    ab1 = ab2 || ab3;
    boolean lb1 = true;
if(fo0 != null){
      fd0 = fo0.m3(fo1, fo0, fo1, fo0, fd1, fd0, fd1, fd0);
}
    Output.points[2][8] -= fd1;
    ab3 = ab4 && fb0;

Thought.STACK_COUNTER++;
return fb1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
        fb0 = fb1 && fb0;
    ad1 = ad2 + ad3;
if(fo1 != null){
      fo1.m1(ad4, fd0, fd1, ad1, fb1, fb0, fb1, fb0);
}
    fb1 = ad2 < ad3;
if(fo0 != null){
      fo0.m2(fo1, fo0, fo1, fo0, fb0, fb1, fb0, fb1);
}
if(fo0 != null){
      fo1 = fo0.m4();
}
    for(int i0=0; i0<10; i0++){
        boolean lb0 = false;
        boolean lb1 = true;
        ad4 = fd0 - fd1;
        lb0 = lb1 || fb0;
        ad1 = ad2 - ad3;
}
Thought.STACK_COUNTER++;
return fb1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Thought lo0 = Thought251.getInstance(fo1, fo0, fo1, fo0, ad2, ad3, ad4, fd0, ab1, ab2, ab3, ab4);
if(fo0 != null){
      fo1 = fo0.m4(fb0, fb1, ab1, ab2);
}
    double ld1 = 261.1141406108374;
if(fo1 != null){
      fd0 = fo1.m3(fo0, fo1, fo0, fo1);
}
    Thought lo2 = Thought303.getInstance(fd1, ld1, ad1, ad2);
    ad3 = ad4 - fd0;
    ab3 = ab4 || fb0;
    for(int i0=0; i0<10; i0++){
        fd1 = ld1 - ad1;
if(fo0 != null){
          fb1 = fo0.m2(fo1, fo0, fo1, fo0, ad2, ad3, ad4, fd0);
}
if(fo1 != null){
          fo1.m3(fd1, ld1, ad1, ad2, ab1, ab2, ab3, ab4);
}
        Output.points[3][0] += ad3;
        ad4 = fd0 + fd1;
        ld1 = ad1 + ad2;
        ad3 = ad4 + fd0;
        fd1 = ld1 + ad1;
}
Thought.STACK_COUNTER++;
return fb0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(Thought ao1, Thought ao2, Thought ao3, Thought ao4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao1 != null){
      ao1.m1(ao2, ao3, ao4, fo0, fb1, fb0, fb1, fb0);
}
if(fo1 != null){
      fb1 = fo1.m2();
}
    Thought lo0 = Thought360.getInstance(ao1, ao2, ao3, ao4, fd0, fd1, fd0, fd1, fb0, fb1, fb0, fb1);
    fb0 = fd0 > fd1;
if(fo0 != null){
      fd0 = fo0.m3(fb1, fb0, fb1, fb0);
}
    Thought lo1 = Thought43.getInstance(fo1, ao1, ao2, ao3);
    double ld2 = 644.177795541712;
if(fo0 != null){
      ao4 = fo0.m4(fd0, fd1, ld2, fd0);
}
if(fo1 != null){
      fd1 = fo1.m3(ao1, ao2, ao3, ao4, ld2, fd0, fd1, ld2);
}
    boolean lb3 = true;
if(fo0 != null){
      fd0 = fo0.m3(fd1, ld2, fd0, fd1, fb0, fb1, lb3, fb0);
}
    ld2 = fd0 - fd1;
    Output.points[3][1] += ld2;
    fd0 *= -1;
if(ao1 != null){
      fo1 = ao1.m4(ao2, ao3, ao4, fo0, fb1, lb3, fb0, fb1);
}
    boolean lb4 = true;
    fd1 = ld2 - fd0;
    fd1 = ld2 - fd0;
    lb3 = fd1 > ld2;
if(fo1 != null){
      fd0 = fo1.m3();
}
    fd1 *= -1;
    lb4 = fb0 || fb1;
if(ao1 != null){
      lb3 = ao1.m2(ao2, ao3, ao4, fo0, ld2, fd0, fd1, ld2, lb4, fb0, fb1, lb3);
}
    lb4 = fd0 > fd1;
    fb0 = ld2 > fd0;
    fd1 *= -1;
    fb1 = lb3 && lb4;
    ld2 = fd0 - fd1;
    double ld5 = 149.9915154970689;

Thought.STACK_COUNTER++;
return fb0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ad1 = ad2 - ad3;
if(ao2 != null){
      ao2.m3(fb1, fb0, fb1, fb0);
}
    ad4 = fd0 - fd1;
    Thought lo0 = Thought18.getInstance(ao3, ao4, fo0, fo1);
if(ao1 != null){
      fb1 = ao1.m2(ad1, ad2, ad3, ad4);
}
    fb0 = fd0 > fd1;
if(ao2 != null){
      ao2.m2(ao3, ao4, fo0, fo1, ad1, ad2, ad3, ad4);
}
if(ao1 != null){
      fd0 = ao1.m3(fd1, ad1, ad2, ad3, fb1, fb0, fb1, fb0);
}
    fb1 = ad4 > fd0;
if(ao2 != null){
      fd1 = ao2.m3(ao3, ao4, fo0, fo1, fb0, fb1, fb0, fb1);
}
if(ao2 != null){
      ao1 = ao2.m4();
}
    Thought lo1 = Thought37.getInstance(ao3, ao4, fo0, fo1, ad1, ad2, ad3, ad4, fb0, fb1, fb0, fb1);
if(ao2 != null){
      ao1 = ao2.m4(fb0, fb1, fb0, fb1);
}
    fd0 = fd1 + ad1;
if(ao4 != null){
      ao3 = ao4.m4(fo0, fo1, ao1, ao2);
}
    ad2 = ad3 + ad4;
    fb0 = fb1 || fb0;
    boolean lb2 = true;
if(ao4 != null){
      ao3 = ao4.m4(fd0, fd1, ad1, ad2);
}
    Thought lo3 = Thought58.getInstance(fo0, fo1, ao1, ao2, ad3, ad4, fd0, fd1);
    ad1 *= -1;
    Output.points[3][2] += ad2;
    double ld4 = 534.6212787862039;
if(ao3 != null){
      ad2 = ao3.m3(ad3, ad4, fd0, fd1, fb0, fb1, lb2, fb0);
}
    fb1 = lb2 && fb0;
    ld4 = ad1 + ad2;
    fb1 = ad3 > ad4;
if(fo0 != null){
      ao4 = fo0.m4(fo1, ao1, ao2, ao3, lb2, fb0, fb1, lb2);
}
if(ao4 != null){
      fb0 = ao4.m2();
}

Thought.STACK_COUNTER++;
return fb1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    double ld0 = 429.96423534566856;
    fd0 *= -1;
    Thought lo1 = Thought150.getInstance(ao1, ao2, ao3, ao4, fd1, ld0, fd0, fd1, ab1, ab2, ab3, ab4);
    ld0 = fd0 + fd1;
    ld0 *= -1;
    fd0 *= -1;
    boolean lb2 = false;
    Thought lo3 = Thought173.getInstance(ab4, fb0, fb1, lb2);
if(fo0 != null){
      ab1 = fo0.m2(fo1, ao1, ao2, ao3);
}
    fd1 = ld0 + fd0;
if(fo0 != null){
      ao4 = fo0.m4(fd1, ld0, fd0, fd1);
}
    boolean lb4 = true;
    Thought lo5 = Thought14.getInstance(fo1, ao1, ao2, ao3, ld0, fd0, fd1, ld0);
    fd0 = fd1 + ld0;
    ab1 = !ab2;
    fd0 *= -1;
    ab3 = fd1 < ld0;
if(ao4 != null){
      ao4.m2(fd0, fd1, ld0, fd0, ab4, fb0, fb1, lb2);
}
    fd1 *= -1;
    ld0 = fd0 + fd1;

Thought.STACK_COUNTER++;
return lb4;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao1 != null){
      ao1.m1(ao2, ao3, ao4, fo0, ab1, ab2, ab3, ab4);
}
if(fo1 != null){
      ad1 = fo1.m3();
}
if(ao1 != null){
      ao1.m2(ao2, ao3, ao4, fo0, ad2, ad3, ad4, fd0, fb0, fb1, ab1, ab2);
}
if(ao1 != null){
      fo1 = ao1.m4(ab3, ab4, fb0, fb1);
}
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1);
}
    double ld0 = 220.83554638065496;
    Output.points[3][3] += fd0;
    double ld1 = 882.596887996146;
    fd0 = fd1 - ld0;
    ld1 = ad1 + ad2;
if(ao2 != null){
      ab1 = ao2.m2(ad3, ad4, fd0, fd1);
}
    Output.points[3][4] -= ld0;
    ab2 = ab3 && ab4;
    Thought lo2 = Thought97.getInstance(ao3, ao4, fo0, fo1, ld1, ad1, ad2, ad3);
if(ao1 != null){
      ad4 = ao1.m3(fd0, fd1, ld0, ld1, fb0, fb1, ab1, ab2);
}
if(ao2 != null){
      ab3 = ao2.m2(ao3, ao4, fo0, fo1, ab4, fb0, fb1, ab1);
}
    ad1 = ad2 - ad3;
    ab2 = !ab3;

Thought.STACK_COUNTER++;
return ab4;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
double m3() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Thought lo0 = Thought209.getInstance();
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, fd1, fd0, fd1, fd0, fb0, fb1, fb0, fb1);
}
    double ld1 = 363.3378660837491;
    fb0 = fd0 < fd1;
    Thought lo2 = Thought72.getInstance(fb1, fb0, fb1, fb0);
    fb1 = fb0 && fb1;
if(fo0 != null){
          fb0 = fo0.m2(fo1, fo0, fo1, fo0);
}
if(fo1 != null){
      ld1 = fo1.m3(fd0, fd1, ld1, fd0);
}
    fd1 *= -1;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, ld1, fd0, fd1, ld1);
}
    fd0 *= -1;
    fd1 = ld1 - fd0;
if(fo0 != null){
      fb1 = fo0.m2(fd1, ld1, fd0, fd1, fb0, fb1, fb0, fb1);
}
    fb0 = fb1 || fb0;
    ld1 = fd0 - fd1;
    double ld3 = 107.60586973093643;
    fb1 = !fb0;
    fb1 = ld1 < ld3;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, fb0, fb1, fb0, fb1);
}
    fd0 = fd1 - ld1;
    ld3 = fd0 - fd1;
    boolean lb4 = true;
if(fo1 != null){
      fo1.m3();
}
    lb4 = fb0 || fb1;
    if (lb4) {
}
Thought.STACK_COUNTER++;
return ld1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Output.points[3][5] += fd1;
    ab2 = !ab3;
    double ld0 = 471.85678349376127;
    Output.points[3][6] += ld0;
    Thought lo1 = Thought81.getInstance(fo0, fo1, fo0, fo1, fd0, fd1, ld0, fd0, ab4, fb0, fb1, ab1);
    Thought lo2 = Thought37.getInstance(ab2, ab3, ab4, fb0);
    fb1 = fd1 < ld0;
    Thought lo3 = Thought345.getInstance(fo0, fo1, fo0, fo1);
    fd0 *= -1;
if(fo0 != null){
      fo0.m3(fd1, ld0, fd0, fd1);
}
if(fo1 != null){
      ld0 = fo1.m3(fo0, fo1, fo0, fo1, fd0, fd1, ld0, fd0);
}
        ab1 = fd1 < ld0;
    boolean lb4 = true;
if(fo1 != null){
      fo0 = fo1.m4(fd0, fd1, ld0, fd0, ab1, ab2, ab3, ab4);
}
if(fo0 != null){
      fd1 = fo0.m3(fo1, fo0, fo1, fo0, fb0, fb1, lb4, ab1);
}
    ab2 = !ab3;
    boolean lb5 = false;
    ld0 = fd0 - fd1;
if(fo0 != null){
      fo1 = fo0.m4();
}

Thought.STACK_COUNTER++;
return ld0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ad2 *= -1;
if(fo1 != null){
      fb0 = fo1.m2(fo0, fo1, fo0, fo1, ad3, ad4, fd0, fd1, fb1, fb0, fb1, fb0);
}
    ad1 *= -1;
if(fo1 != null){
      fo0 = fo1.m4(fb1, fb0, fb1, fb0);
}
    fb1 = fb0 || fb1;
if(fo0 != null){
      fo0.m2(fo1, fo0, fo1, fo0);
}
    ad2 *= -1;
if(fo1 != null){
      fb0 = fo1.m2(ad3, ad4, fd0, fd1);
}
    Output.points[3][7] -= ad1;
    Output.points[3][8] -= ad2;
    boolean lb0 = false;

Thought.STACK_COUNTER++;
return ad3;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ad2 *= -1;
    Output.points[4][0] += ad3;
    ab2 = ad4 > fd0;
    Thought lo0 = Thought165.getInstance(fo0, fo1, fo0, fo1, fd1, ad1, ad2, ad3);
        ab3 = !ab4;
if(fo0 != null){
      ad4 = fo0.m3(fd0, fd1, ad1, ad2, fb0, fb1, ab1, ab2);
}
if(fo1 != null){
      fo1.m1(fo0, fo1, fo0, fo1, ab3, ab4, fb0, fb1);
}
if(fo1 != null){
      fo0 = fo1.m4();
}
    boolean lb1 = true;
    lb1 = ab1 || ab2;
    boolean lb2 = true;
    ad3 = ad4 - fd0;
    boolean lb3 = false;
    ab1 = fd1 > ad1;
    ab2 = ab3 || ab4;
    Thought lo4 = Thought181.getInstance(fo0, fo1, fo0, fo1, ad2, ad3, ad4, fd0, fb0, fb1, lb1, lb2);
    double ld5 = 378.4976993625228;

Thought.STACK_COUNTER++;
return fd0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(Thought ao1, Thought ao2, Thought ao3, Thought ao4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd0 = fd1 - fd0;
    if (fb0) {
        fb1 = !fb0;
        boolean lb0 = true;
        fd1 = fd0 - fd1;
        fd0 *= -1;
        fd1 = fd0 - fd1;
if(ao2 != null){
          ao1 = ao2.m4(fb0, fb1, lb0, fb0);
}
        fb1 = fd0 > fd1;
        Output.points[4][1] -= fd0;
if(ao3 != null){
          fd1 = ao3.m3(ao4, fo0, fo1, ao1);
}
        Output.points[4][2] -= fd0;
        fd1 = fd0 - fd1;
        Output.points[4][3] -= fd0;
if(ao2 != null){
          lb0 = ao2.m2(fd1, fd0, fd1, fd0);
}
        boolean lb1 = false;
        lb1 = fd1 < fd0;
        boolean lb2 = true;
if(ao3 != null){
          fd1 = ao3.m3(ao4, fo0, fo1, ao1, fd0, fd1, fd0, fd1);
}
        fd0 = fd1 - fd0;
if(ao2 != null){
          fd1 = ao2.m3(fd0, fd1, fd0, fd1, lb2, fb0, fb1, lb0);
}
        lb1 = lb2 && fb0;
        fb1 = lb0 || lb1;
        lb2 = !fb0;
        fb1 = lb0 || lb1;
        Output.points[4][4] -= fd0;
        fd1 = fd0 - fd1;
}
Thought.STACK_COUNTER++;
return fd0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Output.points[4][5] += ad2;
if(ao2 != null){
      ao1 = ao2.m4(ao3, ao4, fo0, fo1, fb0, fb1, fb0, fb1);
}
    double ld0 = 436.62525786491506;
if(ao1 != null){
      fb0 = ao1.m2();
}
    Output.points[4][6] += ad2;
if(ao2 != null){
      fb1 = ao2.m2(ao3, ao4, fo0, fo1, ad3, ad4, fd0, fd1, fb0, fb1, fb0, fb1);
}
    Output.points[4][7] += ld0;
    fb0 = !fb1;
if(ao1 != null){
      ao1.m1(fb0, fb1, fb0, fb1);
}
    Thought lo1 = Thought75.getInstance(ao2, ao3, ao4, fo0);
if(ao1 != null){
      fo1 = ao1.m4(ad1, ad2, ad3, ad4);
}
    if (fb0) {
if(ao3 != null){
          ao2 = ao3.m4(ao4, fo0, fo1, ao1, fd0, fd1, ld0, ad1);
}
}
Thought.STACK_COUNTER++;
return ad2;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd1 *= -1;
    fd0 = fd1 - fd0;
    ab2 = !ab3;
    ab4 = fd1 > fd0;
    fb0 = fb1 && ab1;
    fd1 *= -1;
if(ao3 != null){
      ao2 = ao3.m4(fd0, fd1, fd0, fd1, ab2, ab3, ab4, fb0);
}
    Thought lo0 = Thought310.getInstance(ao4, fo0, fo1, ao1, fb1, ab1, ab2, ab3);
    double ld1 = 89.81993485365228;
    ld1 = fd0 - fd1;
if(ao3 != null){
      ao2 = ao3.m4();
}
    boolean lb2 = true;

Thought.STACK_COUNTER++;
return ld1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ab2 = ad2 > ad3;
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, ad4, fd0, fd1, ad1, ab3, ab4, fb0, fb1);
}
if(ao2 != null){
      ao2.m2(ab1, ab2, ab3, ab4);
}
    double ld0 = 76.34434995626744;
    Output.points[4][8] += ad1;
    Thought lo1 = Thought332.getInstance(ao3, ao4, fo0, fo1);
if(ao2 != null){
      ao1 = ao2.m4(ad2, ad3, ad4, fd0);
}
    fd1 *= -1;
    fb0 = fb1 || ab1;
if(ao4 != null){
      ao3 = ao4.m4(fo0, fo1, ao1, ao2, ld0, ad1, ad2, ad3);
}

Thought.STACK_COUNTER++;
return ad4;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
Thought m4() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(fo0 != null){
      fd1 = fo0.m3(fd0, fd1, fd0, fd1, fb1, fb0, fb1, fb0);
}
    fb1 = fb0 || fb1;
    fd0 *= -1;
    boolean lb0 = false;
    boolean lb1 = false;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, lb0, lb1, fb0, fb1);
}
if(fo0 != null){
      fo1 = fo0.m4();
}
if(fo1 != null){
      fo1.m1(fo0, fo1, fo0, fo1, fd1, fd0, fd1, fd0, lb0, lb1, fb0, fb1);
}
    fd1 *= -1;
    Thought lo2 = Thought150.getInstance(lb0, lb1, fb0, fb1);
    boolean lb3 = false;
    double ld4 = 659.5548556254297;
        Thought lo5 = Thought64.getInstance(fo0, fo1, fo0, fo1);
if(fo0 != null){
          fo0.m2(ld4, fd0, fd1, ld4);
}
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, fd0, fd1, ld4, fd0);
}
    fd1 = ld4 - fd0;

Thought.STACK_COUNTER++;
return fo1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ab1 = !ab2;
    ab3 = !ab4;
    fb0 = !fb1;
    ab1 = ab2 && ab3;
    Output.points[5][0] -= fd0;
if(fo1 != null){
      fo0 = fo1.m4(fd1, fd0, fd1, fd0, ab4, fb0, fb1, ab1);
}
    fd1 = fd0 + fd1;
    fd0 = fd1 + fd0;
    fd1 *= -1;
    ab2 = fd0 > fd1;
if(fo0 != null){
      fo0.m3(fo1, fo0, fo1, fo0, ab3, ab4, fb0, fb1);
}
    Thought lo0 = Thought77.getInstance();

Thought.STACK_COUNTER++;
return fo1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb0 = fb1 && fb0;
    fb1 = ad1 > ad2;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, ad3, ad4, fd0, fd1, fb0, fb1, fb0, fb1);
}
if(fo0 != null){
      ad1 = fo0.m3(fb0, fb1, fb0, fb1);
}
    ad2 *= -1;
if(fo1 != null){
      fo1.m2(fo0, fo1, fo0, fo1);
}
if(fo0 != null){
      fo0.m2(ad3, ad4, fd0, fd1);
}
    ad1 *= -1;
    boolean lb0 = true;
    boolean lb1 = true;
    boolean lb2 = false;
    ad2 *= -1;
    ad3 *= -1;
    lb0 = ad4 < fd0;
    fd1 = ad1 - ad2;
if(fo0 != null){
          fo1 = fo0.m4(fo1, fo0, fo1, fo0, ad3, ad4, fd0, fd1);
}
    double ld3 = 87.88998451856762;
if(fo0 != null){
      fo1 = fo0.m4(ld3, ad1, ad2, ad3, lb1, lb2, fb0, fb1);
}
    Output.points[5][1] -= ad4;
    lb0 = lb1 || lb2;
    fd0 = fd1 + ld3;
if(fo1 != null){
      ad1 = fo1.m3(fo0, fo1, fo0, fo1, fb0, fb1, lb0, lb1);
}

Thought.STACK_COUNTER++;
return fo0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(fo1 != null){
      ad1 = fo1.m3();
}
if(fo0 != null){
      fo0.m2(fo1, fo0, fo1, fo0, ad2, ad3, ad4, fd0, ab1, ab2, ab3, ab4);
}
if(fo0 != null){
      fo1 = fo0.m4(fb0, fb1, ab1, ab2);
}
if(fo1 != null){
      fd1 = fo1.m3(fo0, fo1, fo0, fo1);
}
if(fo0 != null){
      ab3 = fo0.m2(ad1, ad2, ad3, ad4);
}
    fd0 = fd1 - ad1;
    ab4 = !fb0;
    Output.points[5][2] -= ad2;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, ad3, ad4, fd0, fd1);
}
    Output.points[5][3] -= ad1;
if(fo1 != null){
      ad2 = fo1.m3(ad3, ad4, fd0, fd1, fb1, ab1, ab2, ab3);
}
    ab4 = !fb0;
    fb1 = ad1 > ad2;
if(fo0 != null){
      ab1 = fo0.m2(fo1, fo0, fo1, fo0, ab2, ab3, ab4, fb0);
}
if(fo1 != null){
      fo1.m3();
}
    fb1 = ab1 || ab2;
if(fo0 != null){
      ad3 = fo0.m3(fo1, fo0, fo1, fo0, ad4, fd0, fd1, ad1, ab3, ab4, fb0, fb1);
}
    boolean lb0 = true;
    for(int i0=0; i0<10; i0++){
        ad2 = ad3 + ad4;
        fd0 = fd1 + ad1;
        Thought lo1 = Thought212.getInstance(lb0, ab1, ab2, ab3);
}
Thought.STACK_COUNTER++;
return fo1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(Thought ao1, Thought ao2, Thought ao3, Thought ao4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb0 = fd1 < fd0;
    fd1 *= -1;
    fb1 = fb0 || fb1;
if(ao1 != null){
      ao1.m3(ao2, ao3, ao4, fo0);
}
    fb0 = fb1 && fb0;
if(ao1 != null){
      fo1 = ao1.m4(fd0, fd1, fd0, fd1);
}
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, fd0, fd1, fd0, fd1);
}
if(ao2 != null){
      fb1 = ao2.m2(fd0, fd1, fd0, fd1, fb0, fb1, fb0, fb1);
}
    fd0 = fd1 - fd0;
    fd1 = fd0 + fd1;
if(ao4 != null){
      ao3 = ao4.m4(fo0, fo1, ao1, ao2, fb0, fb1, fb0, fb1);
}
    Output.points[5][4] += fd0;
    boolean lb0 = true;
    boolean lb1 = true;
    Thought lo2 = Thought297.getInstance();
    Thought lo3 = Thought229.getInstance(ao3, ao4, fo0, fo1, fd1, fd0, fd1, fd0, lb0, lb1, fb0, fb1);
if(ao1 != null){
      lb0 = ao1.m2(lb1, fb0, fb1, lb0);
}
    fd1 = fd0 - fd1;
if(ao2 != null){
      fd0 = ao2.m3(ao3, ao4, fo0, fo1);
}
    double ld4 = 647.9376959606058;
    Thought lo5 = Thought138.getInstance(fd0, fd1, ld4, fd0);
if(ao1 != null){
      lb1 = ao1.m2(ao2, ao3, ao4, fo0, fd1, ld4, fd0, fd1);
}
    fb0 = fb1 && lb0;
    boolean lb6 = false;
if(fo1 != null){
      lb1 = fo1.m2(ld4, fd0, fd1, ld4, lb6, fb0, fb1, lb0);
}
        fd0 = fd1 + ld4;
    boolean lb7 = false;

Thought.STACK_COUNTER++;
return ao1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Thought lo0 = Thought78.getInstance(ao2, ao3, ao4, fo0, fb1, fb0, fb1, fb0);
    ad2 = ad3 - ad4;
    fd0 *= -1;
    fd1 = ad1 + ad2;
    fb1 = ad3 < ad4;
        fb0 = fd0 < fd1;
    ad1 *= -1;
    double ld1 = 73.20887424780018;
    boolean lb2 = false;
    fb0 = fb1 && lb2;
    Thought lo3 = Thought52.getInstance();

Thought.STACK_COUNTER++;
return fo1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} 
}
Thought m4(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao2 != null){
      ao1 = ao2.m4(ao3, ao4, fo0, fo1, fd1, fd0, fd1, fd0, ab2, ab3, ab4, fb0);
}
    double ld0 = 489.2239037191202;
    boolean lb1 = false;
    fd0 = fd1 + ld0;
    Thought lo2 = Thought381.getInstance(fb0, fb1, lb1, ab1);
    Output.points[5][5] += fd0;
    Output.points[5][6] += fd1;
    double ld3 = 459.40977039779546;
    Thought lo4 = Thought63.getInstance(ao1, ao2, ao3, ao4);
    ld0 = ld3 - fd0;
    Thought lo5 = Thought24.getInstance(fd1, ld0, ld3, fd0);
    fd1 = ld0 + ld3;
    double ld6 = 952.145586533783;
if(fo0 != null){
      fo0.m3(fo1, ao1, ao2, ao3, ld6, fd0, fd1, ld0);
}
    boolean lb7 = true;
    ld3 *= -1;

Thought.STACK_COUNTER++;
return ao4;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao1 != null){
      ab1 = ao1.m2(ad1, ad2, ad3, ad4, ab2, ab3, ab4, fb0);
}
    Thought lo0 = Thought324.getInstance(ao2, ao3, ao4, fo0, fb1, ab1, ab2, ab3);
if(ao1 != null){
      fo1 = ao1.m4();
}
    fd0 = fd1 - ad1;
if(ao2 != null){
      ao2.m1(ao3, ao4, fo0, fo1, ad2, ad3, ad4, fd0, ab4, fb0, fb1, ab1);
}
    fd1 = ad1 - ad2;
    double ld1 = 717.0061792486343;
    ab2 = ab3 && ab4;
    fb0 = !fb1;
    ab1 = ab2 && ab3;
    ad2 *= -1;
    ad3 = ad4 - fd0;
if(ao2 != null){
      ao1 = ao2.m4(ab4, fb0, fb1, ab1);
}
    boolean lb2 = true;
    fd1 = ld1 - ad1;
    Output.points[5][7] += ad2;
    ab1 = ab2 && ab3;
    ad3 *= -1;
if(ao3 != null){
      ab4 = ao3.m2(ao4, fo0, fo1, ao1);
}
    double ld3 = 831.6605672268319;
    fb0 = fb1 && lb2;

Thought.STACK_COUNTER++;
return ao2;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
void accept(String color) throws CountDownExc {
switch (color) {
case "e":
empty();
break;
case "own":
own();
break;
case "other":
other();
break;
default:
throw new RuntimeException("Illegal color!!!");
}
}
void empty() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(fo1 != null){
      fo0 = fo1.m4(fd0, fd1, fd0, fd1);
}
if(fo0 != null){
      fb1 = fo0.m2(fo1, fo0, fo1, fo0, fd0, fd1, fd0, fd1);
}
    Output.points[5][8] += fd0;
    fd1 = fd0 - fd1;
    fb0 = fd0 < fd1;
    fb1 = fb0 || fb1;
    fd0 *= -1;
if(fo0 != null){
      fo1 = fo0.m4(fd1, fd0, fd1, fd0, fb0, fb1, fb0, fb1);
}
    Output.points[6][0] += fd1;
        fd0 = fd1 + fd0;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, fb0, fb1, fb0, fb1);
}
    fb0 = fb1 || fb0;
if(fo0 != null){
      fo1 = fo0.m4();
}
    double ld0 = 213.76498502252497;
    fb1 = fd0 > fd1;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, ld0, fd0, fd1, ld0, fb0, fb1, fb0, fb1);
}
    fd0 = fd1 + ld0;
if(fo1 != null){
      fd0 = fo1.m3(fb0, fb1, fb0, fb1);
}
    fd1 = ld0 + fd0;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void own() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Thought lo0 = Thought116.getInstance(fo0, fo1, fo0, fo1);
if(fo1 != null){
      fo0 = fo1.m4(fd0, fd1, fd0, fd1);
}
    fb0 = fb1 && fb0;
    fd0 = fd1 + fd0;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, fd1, fd0, fd1, fd0);
}
    boolean lb1 = true;
    double ld2 = 800.6540701331377;
    boolean lb3 = true;
    fd0 *= -1;
if(fo0 != null){
      lb3 = fo0.m2(fd1, ld2, fd0, fd1, fb0, fb1, lb1, lb3);
}
    fb0 = fb1 || lb1;
    Thought lo4 = Thought335.getInstance(fo1, fo0, fo1, fo0, lb3, fb0, fb1, lb1);
    Output.points[6][1] += ld2;
    fd0 *= -1;
if(fo1 != null){
      fo1.m1();
}
    fd1 = ld2 + fd0;
    Output.points[6][2] -= fd1;
    lb3 = fb0 || fb1;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, ld2, fd0, fd1, ld2, lb1, lb3, fb0, fb1);
}
    Output.points[6][3] -= fd0;
if(fo0 != null){
      lb1 = fo0.m2(lb3, fb0, fb1, lb1);
}
    fd1 = ld2 - fd0;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0);
}
if(fo1 != null){
      fd1 = fo1.m3(ld2, fd0, fd1, ld2);
}
    lb3 = fd0 < fd1;
    Thought lo5 = Thought176.getInstance(fo0, fo1, fo0, fo1, ld2, fd0, fd1, ld2);
    boolean lb6 = false;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void other() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    double ld0 = 86.15943266567577;
    double ld1 = 942.992472392752;
    Thought lo2 = Thought253.getInstance(ld1, fd0, fd1, ld0, fb0, fb1, fb0, fb1);
    ld1 *= -1;
    fd0 = fd1 + ld0;
    double ld3 = 667.7037369495388;
    fb0 = ld1 < ld3;
    double ld4 = 493.6079693750159;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, fb1, fb0, fb1, fb0);
}
    double ld5 = 816.6548450009659;
    fb1 = fb0 || fb1;
    boolean lb6 = true;
if(fo0 != null){
      ld4 = fo0.m3();
}
    boolean lb7 = false;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
}
