package genetic;
import java.util.ArrayList;
class Thought73 extends Thought{
private static ArrayList<Thought73> instances = new ArrayList<>();
private static int instanceCounter = 0;
private boolean fb0 = false;
private boolean fb1 = true;
private double fd0 = 156.95533177171083;
private double fd1 = 135.9007224346557;
private Thought fo0 = null;
private Thought fo1 = null;
Thought73 (){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
Thought.STACK_COUNTER++;
}
Thought73 (boolean ab1, boolean ab2, boolean ab3, boolean ab4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fb0 = ab1; 
fb1 = ab2; 
Thought.STACK_COUNTER++;
}
Thought73 (double ad1, double ad2, double ad3, double ad4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fd0 = ad1; 
fd1 = ad2; 
Thought.STACK_COUNTER++;
}
Thought73 (double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fb0 = ab1; 
fb1 = ab2; 
fd0 = ad1; 
fd1 = ad2; 
Thought.STACK_COUNTER++;
}
Thought73 (Thought ao1, Thought ao2, Thought ao3, Thought ao4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fo0 = ao1; 
fo1 = ao2; 
Thought.STACK_COUNTER++;
}
Thought73 (Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fd0 = ad1; 
fd1 = ad2; 
fo0 = ao1; 
fo1 = ao2; 
Thought.STACK_COUNTER++;
}
Thought73 (Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fb0 = ab1; 
fb1 = ab2; 
fo0 = ao1; 
fo1 = ao2; 
Thought.STACK_COUNTER++;
}
Thought73 (Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
fb0 = ab1; 
fb1 = ab2; 
fd0 = ad1; 
fd1 = ad2; 
fo0 = ao1; 
fo1 = ao2; 
Thought.STACK_COUNTER++;
}
public static Thought73 getInstance(){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought73 instance = new Thought73 (); 
instances.add(instance); 
return instance; 
} 
}
public static Thought73 getInstance(boolean ab1, boolean ab2, boolean ab3, boolean ab4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought73 instance = new Thought73 ( ab1,  ab2,  ab3,  ab4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought73 getInstance(double ad1, double ad2, double ad3, double ad4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought73 instance = new Thought73 ( ad1,  ad2,  ad3,  ad4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought73 getInstance(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought73 instance = new Thought73 ( ad1,  ad2,  ad3,  ad4,  ab1,  ab2,  ab3,  ab4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought73 getInstance(Thought ao1, Thought ao2, Thought ao3, Thought ao4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought73 instance = new Thought73 ( ao1,  ao2,  ao3,  ao4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought73 getInstance(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought73 instance = new Thought73 ( ao1,  ao2,  ao3,  ao4,  ad1,  ad2,  ad3,  ad4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought73 getInstance(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought73 instance = new Thought73 ( ao1,  ao2,  ao3,  ao4,  ab1,  ab2,  ab3,  ab4); 
instances.add(instance); 
return instance; 
} 
}
public static Thought73 getInstance(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4){
if (instances.size() >= 10) { 
instanceCounter++; 
instanceCounter%=10; 
return instances.get(instanceCounter); 
} else { 
Thought73 instance = new Thought73 ( ao1,  ao2,  ao3,  ao4,  ad1,  ad2,  ad3,  ad4,  ab1,  ab2,  ab3,  ab4); 
instances.add(instance); 
return instance; 
} 
}
void m1() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(fo0 != null){
      fb0 = fo0.m2(fd1, fd0, fd1, fd0);
}
    fd1 = fd0 + fd1;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, fd0, fd1, fd0, fd1);
}
    fd0 = fd1 - fd0;
if(fo1 != null){
      fo1.m3(fb1, fb0, fb1, fb0);
}
    boolean lb0 = true;
    fd1 = fd0 - fd1;
        fb0 = !fb1;
if(fo1 != null){
      fo0 = fo1.m4();
}
    fd0 = fd1 + fd0;
if(fo1 != null){
      fo0 = fo1.m4(fd1, fd0, fd1, fd0, lb0, fb0, fb1, lb0);
}
    boolean lb1 = false;
if(fo1 != null){
          fo0 = fo1.m4(fo0, fo1, fo0, fo1);
}
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, fd1, fd0, fd1, fd0, lb1, fb0, fb1, lb0);
}
    double ld2 = 412.02934702147763;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, lb1, fb0, fb1, lb0);
}
if(fo1 != null){
      fo0 = fo1.m4(fd0, fd1, ld2, fd0);
}
    Thought lo3 = Thought4.getInstance(fo0, fo1, fo0, fo1, fd1, ld2, fd0, fd1);
    ld2 *= -1;
    lb1 = fd0 > fd1;
    fb0 = fb1 || lb0;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd0 = fd1 + fd0;
    boolean lb0 = true;
    fd1 *= -1;
    Thought lo1 = Thought57.getInstance(ab1, ab2, ab3, ab4);
    fb0 = fd0 > fd1;
        Output.points[0][5] += fd0;
if(fo1 != null){
      fo0 = fo1.m4();
}
if(fo0 != null){
      fo0.m2(fd1, fd0, fd1, fd0, fb1, lb0, ab1, ab2);
}
    ab3 = fd1 < fd0;
    double ld2 = 627.8673182311942;
    fd0 = fd1 - ld2;
        ab4 = !fb0;
    fd0 = fd1 + ld2;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(fo1 != null){
      fo1.m2(fo0, fo1, fo0, fo1);
}
if(fo0 != null){
      fo0.m1(fo1, fo0, fo1, fo0, ad2, ad3, ad4, fd0, fb0, fb1, fb0, fb1);
}
    Output.points[0][6] += fd1;
    ad1 = ad2 - ad3;
    fb0 = ad4 < fd0;
    boolean lb0 = false;
        Thought lo1 = Thought62.getInstance(fo1, fo0, fo1, fo0, fb0, fb1, lb0, fb0);
if(fo1 != null){
      fd1 = fo1.m3(ad1, ad2, ad3, ad4);
}
if(fo0 != null){
      fo0.m3(fo1, fo0, fo1, fo0, fd0, fd1, ad1, ad2);
}
    fb1 = ad3 > ad4;
if(fo0 != null){
      fo1 = fo0.m4(lb0, fb0, fb1, lb0);
}
    boolean lb2 = false;
if(fo0 != null){
      fo1 = fo0.m4();
}
    fd0 = fd1 + ad1;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ab2 = ad2 > ad3;
    double ld0 = 869.7865414014317;
if(fo0 != null){
      fo1 = fo0.m4(ad3, ad4, fd0, fd1, ab3, ab4, fb0, fb1);
}
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0);
}
    ld0 *= -1;
    ab1 = !ab2;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, ad1, ad2, ad3, ad4, ab3, ab4, fb0, fb1);
}
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, ab1, ab2, ab3, ab4);
}
    fb0 = fd0 < fd1;
    double ld1 = 814.730989460413;
    double ld2 = 400.53865400063717;
if(fo1 != null){
      fo1.m1(ld0, ld1, ld2, ad1);
}
    ad2 *= -1;
    ad3 = ad4 - fd0;
    fd1 = ld0 - ld1;
if(fo0 != null){
      fo0.m1(fo1, fo0, fo1, fo0, ld2, ad1, ad2, ad3);
}
    boolean lb3 = true;
    boolean lb4 = false;
if(fo1 != null){
      fo1.m1(ab4, fb0, fb1, lb3);
}
    ad4 = fd0 + fd1;
if(fo1 != null){
      fo0 = fo1.m4();
}
    Thought lo5 = Thought86.getInstance(ld0, ld1, ld2, ad1, lb4, ab1, ab2, ab3);
    ab4 = ad2 < ad3;
    double ld6 = 567.8581893553244;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1);
}
if(fo0 != null){
      ad3 = fo0.m3(fo1, fo0, fo1, fo0, ad4, fd0, fd1, ld0, fb0, fb1, lb3, lb4);
}
    ab1 = !ab2;
        ld1 = ld2 + ld6;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(Thought ao1, Thought ao2, Thought ao3, Thought ao4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb0 = !fb1;
    fb0 = fb1 || fb0;
    boolean lb0 = true;
    fb0 = !fb1;
    Thought lo1 = Thought38.getInstance(ao2, ao3, ao4, fo0, lb0, fb0, fb1, lb0);
if(fo1 != null){
      fd0 = fo1.m3(fd1, fd0, fd1, fd0);
}
if(ao2 != null){
      ao1 = ao2.m4(ao3, ao4, fo0, fo1, fd1, fd0, fd1, fd0);
}
    fd1 = fd0 - fd1;
    Thought lo2 = Thought89.getInstance(fb0, fb1, lb0, fb0);
    fb1 = fd0 < fd1;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb0 = fb1 || fb0;
    boolean lb0 = true;
    boolean lb1 = true;
    ad1 = ad2 + ad3;
if(ao2 != null){
      ao1 = ao2.m4();
}
if(ao3 != null){
          lb1 = ao3.m2(ad4, fd0, fd1, ad1, fb0, fb1, lb0, lb1);
}
if(fo0 != null){
      ao4 = fo0.m4(fo1, ao1, ao2, ao3);
}
    double ld2 = 931.210881644572;
    fb0 = ad1 < ad2;
    for(int i0=0; i0<10; i0++){
if(fo0 != null){
          ao4 = fo0.m4(fo1, ao1, ao2, ao3, ad3, ad4, fd0, fd1, fb1, lb0, lb1, fb0);
}
        Thought lo3 = Thought56.getInstance(ao4, fo0, fo1, ao1, fb1, lb0, lb1, fb0);
if(ao2 != null){
          fb1 = ao2.m2(ld2, ad1, ad2, ad3);
}
if(ao3 != null){
          ao3.m2(ao4, fo0, fo1, ao1, ad4, fd0, fd1, ld2);
}
        ad1 *= -1;
        double ld4 = 806.2004944493142;
        boolean lb5 = false;
if(ao2 != null){
          lb5 = ao2.m2(lb0, lb1, fb0, fb1);
}
        Output.points[0][7] += ad1;
        ad2 = ad3 - ad4;
if(ao3 != null){
          ao3.m3();
}
}
Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd0 = fd1 + fd0;
    ab1 = ab2 || ab3;
    ab4 = !fb0;
    Thought lo0 = Thought30.getInstance(fd1, fd0, fd1, fd0, fb1, ab1, ab2, ab3);
if(ao2 != null){
      ao2.m3(ao3, ao4, fo0, fo1);
}
    ab4 = fd1 > fd0;
if(ao1 != null){
      ao1.m1(ao2, ao3, ao4, fo0, fd1, fd0, fd1, fd0, fb0, fb1, ab1, ab2);
}
    ab3 = fd1 > fd0;
    Output.points[0][8] += fd1;
if(fo1 != null){
      fd0 = fo1.m3(ao1, ao2, ao3, ao4, ab4, fb0, fb1, ab1);
}
    double ld1 = 498.68166866031555;
    fd0 *= -1;
if(fo0 != null){
      fd1 = fo0.m3(ld1, fd0, fd1, ld1);
}
if(ao1 != null){
      fo1 = ao1.m4(ao2, ao3, ao4, fo0, fd0, fd1, ld1, fd0);
}

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void m1(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    boolean lb0 = true;
if(ao3 != null){
      ao2 = ao3.m4(ab1, ab2, ab3, ab4);
}
    Output.points[1][0] -= ad1;
    boolean lb1 = true;
if(fo0 != null){
      ao4 = fo0.m4();
}
    Output.points[1][1] -= ad2;
    ab4 = fb0 && fb1;
    boolean lb2 = true;
    lb0 = lb1 && lb2;
if(fo1 != null){
      ad3 = fo1.m3(ad4, fd0, fd1, ad1, ab1, ab2, ab3, ab4);
}
    Output.points[1][2] -= ad2;
if(ao2 != null){
      ao1 = ao2.m4(ao3, ao4, fo0, fo1);
}
if(ao1 != null){
      ad3 = ao1.m3(ao2, ao3, ao4, fo0, ad4, fd0, fd1, ad1, fb0, fb1, lb0, lb1);
}
    ad2 *= -1;
    double ld3 = 176.72161621371703;
if(fo1 != null){
      ad2 = fo1.m3(ao1, ao2, ao3, ao4, lb2, ab1, ab2, ab3);
}
if(fo0 != null){
      ad3 = fo0.m3(ad4, fd0, fd1, ld3);
}
    ab4 = fb0 && fb1;
        ad1 *= -1;
    ad2 *= -1;
if(ao1 != null){
      fo1 = ao1.m4(ao2, ao3, ao4, fo0, ad3, ad4, fd0, fd1);
}
    double ld4 = 728.3115654357376;
if(ao1 != null){
      fo1 = ao1.m4(lb0, lb1, lb2, ab1);
}

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
boolean m2() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb0 = !fb1;
    fd0 = fd1 - fd0;
    Output.points[1][3] -= fd1;
if(fo0 != null){
      fo1 = fo0.m4();
}
    Thought lo0 = Thought11.getInstance(fd0, fd1, fd0, fd1, fb0, fb1, fb0, fb1);
if(fo1 != null){
      fb0 = fo1.m2(fo0, fo1, fo0, fo1);
}
    double ld1 = 123.66217993952256;
    ld1 = fd0 + fd1;
if(fo0 != null){
      fo0.m3(fo1, fo0, fo1, fo0, ld1, fd0, fd1, ld1, fb1, fb0, fb1, fb0);
}
    boolean lb2 = false;
    for(int i0=0; i0<10; i0++){
        fd0 = fd1 - ld1;
        fb0 = fb1 && lb2;
        fd0 = fd1 + ld1;
if(fo1 != null){
          fo1.m2(fo0, fo1, fo0, fo1, fb0, fb1, lb2, fb0);
}
        double ld3 = 567.4352240049391;
        boolean lb4 = true;
if(fo1 != null){
          fo0 = fo1.m4(ld1, fd0, fd1, ld3);
}
        Thought lo5 = Thought84.getInstance(fo0, fo1, fo0, fo1, ld1, fd0, fd1, ld3);
        fb0 = fb1 && lb4;
        boolean lb6 = true;
if(fo1 != null){
          fo0 = fo1.m4(lb6, lb2, fb0, fb1);
}
        lb4 = ld1 > fd0;
        boolean lb7 = true;
        lb6 = lb7 || lb2;
}
Thought.STACK_COUNTER++;
return fb0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(fo1 != null){
      fo0 = fo1.m4();
}
    fd0 = fd1 + fd0;
    ab1 = ab2 || ab3;
if(fo1 != null){
      fo0 = fo1.m4(fd1, fd0, fd1, fd0, ab4, fb0, fb1, ab1);
}
    fd1 = fd0 - fd1;
    ab2 = fd0 < fd1;
    fd0 *= -1;
if(fo0 != null){
      fo0.m1(fo1, fo0, fo1, fo0);
}
    Thought lo0 = Thought76.getInstance(fo1, fo0, fo1, fo0, fd1, fd0, fd1, fd0, ab3, ab4, fb0, fb1);
    boolean lb1 = false;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, lb1, ab1, ab2, ab3);
}
    ab4 = !fb0;
    fb1 = fd1 > fd0;
    lb1 = ab1 && ab2;
    ab3 = fd1 < fd0;
    ab4 = fb0 || fb1;
    double ld2 = 176.1608111434521;
    lb1 = ab1 || ab2;

Thought.STACK_COUNTER++;
return ab3;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    double ld0 = 620.1618702937524;
    Thought lo1 = Thought27.getInstance(ad1, ad2, ad3, ad4);
    fb0 = !fb1;
if(fo1 != null){
      fo1.m2(fo0, fo1, fo0, fo1, fd0, fd1, ld0, ad1);
}
    ad2 = ad3 - ad4;
    fb0 = fd0 > fd1;
    boolean lb2 = true;
    boolean lb3 = true;
if(fo0 != null){
      ld0 = fo0.m3(lb3, fb0, fb1, lb2);
}
    ad1 = ad2 + ad3;
if(fo1 != null){
      ad4 = fo1.m3();
}
    lb3 = fb0 || fb1;
if(fo0 != null){
      fd0 = fo0.m3(fd1, ld0, ad1, ad2, lb2, lb3, fb0, fb1);
}
    Thought lo4 = Thought67.getInstance(fo1, fo0, fo1, fo0);
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, ad3, ad4, fd0, fd1, lb2, lb3, fb0, fb1);
}
    if (lb2) {
        Thought lo5 = Thought84.getInstance(fo1, fo0, fo1, fo0, lb3, fb0, fb1, lb2);
        boolean lb6 = true;
        lb2 = lb3 || fb0;
        Thought lo7 = Thought1.getInstance(ld0, ad1, ad2, ad3);
if(fo0 != null){
          fo1 = fo0.m4(fo1, fo0, fo1, fo0, ad4, fd0, fd1, ld0);
}
}
Thought.STACK_COUNTER++;
return lb2;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Thought lo0 = Thought22.getInstance(ab2, ab3, ab4, fb0);
    ad2 = ad3 + ad4;
    fb1 = fd0 < fd1;
if(fo0 != null){
      fo1 = fo0.m4();
}
    ab1 = ad1 < ad2;
if(fo0 != null){
      fo1 = fo0.m4(ad3, ad4, fd0, fd1, ab2, ab3, ab4, fb0);
}
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0);
}
    double ld1 = 141.96593125851743;
    ld1 *= -1;
    ad1 = ad2 - ad3;
    double ld2 = 876.146729448776;
    Output.points[1][4] -= ad3;
    ad4 = fd0 + fd1;
    Output.points[1][5] += ld1;
    fb1 = ld2 > ad1;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0, ad2, ad3, ad4, fd0, ab1, ab2, ab3, ab4);
}
    Output.points[1][6] -= fd1;
    double ld3 = 582.7602725438402;
    Output.points[1][7] += ld1;

Thought.STACK_COUNTER++;
return fb0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(Thought ao1, Thought ao2, Thought ao3, Thought ao4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, fb1, fb0, fb1, fb0);
}
    Output.points[1][8] += fd1;
if(ao2 != null){
      ao2.m2(fd0, fd1, fd0, fd1);
}
    fd0 *= -1;
    fb1 = fd1 > fd0;
    fb0 = fd1 > fd0;
if(ao4 != null){
      ao3 = ao4.m4(fo0, fo1, ao1, ao2, fd1, fd0, fd1, fd0);
}
    fd1 *= -1;
    fb1 = fd0 < fd1;
if(ao4 != null){
      ao3 = ao4.m4(fb0, fb1, fb0, fb1);
}
if(fo1 != null){
      fo0 = fo1.m4();
}
    fd0 *= -1;
if(ao2 != null){
      ao1 = ao2.m4(fd1, fd0, fd1, fd0, fb0, fb1, fb0, fb1);
}
if(ao3 != null){
      fd1 = ao3.m3(ao4, fo0, fo1, ao1);
}
    fb0 = !fb1;
    fb0 = fb1 && fb0;
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, fd0, fd1, fd0, fd1, fb1, fb0, fb1, fb0);
}
    fd0 = fd1 - fd0;
    Output.points[2][0] -= fd1;
    double ld0 = 975.0645886555313;
    boolean lb1 = false;
    Output.points[2][1] -= ld0;
    double ld2 = 416.65009559803036;
    boolean lb3 = true;
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, lb3, fb0, fb1, lb1);
}
if(ao2 != null){
      ld2 = ao2.m3(fd0, fd1, ld0, ld2);
}
    lb3 = !fb0;

Thought.STACK_COUNTER++;
return fb1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    double ld0 = 996.7246092454634;
    fb0 = fb1 || fb0;
if(ao1 != null){
      fb1 = ao1.m2(ao2, ao3, ao4, fo0, ld0, ad1, ad2, ad3);
}
    boolean lb1 = false;
    Thought lo2 = Thought23.getInstance(lb1, fb0, fb1, lb1);
if(ao1 != null){
      fo1 = ao1.m4();
}
if(ao2 != null){
      ao2.m2(ad4, fd0, fd1, ld0, fb0, fb1, lb1, fb0);
}
if(ao3 != null){
      ad1 = ao3.m3(ao4, fo0, fo1, ao1);
}
    fb1 = lb1 || fb0;
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, ad2, ad3, ad4, fd0, fb1, lb1, fb0, fb1);
}
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, lb1, fb0, fb1, lb1);
}
    Output.points[2][2] -= fd1;
    fb0 = ld0 < ad1;
    boolean lb3 = false;
if(ao3 != null){
      ao2 = ao3.m4(ad2, ad3, ad4, fd0);
}
    fd1 = ld0 + ad1;
    Thought lo4 = Thought81.getInstance(ao4, fo0, fo1, ao1, ad2, ad3, ad4, fd0);
    fb0 = fd1 > ld0;
if(ao3 != null){
      ao2 = ao3.m4(fb1, lb1, lb3, fb0);
}

Thought.STACK_COUNTER++;
return fb1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd1 = fd0 + fd1;
    fd0 = fd1 - fd0;
if(ao3 != null){
      ao2 = ao3.m4();
}
    double ld0 = 391.7605226356738;
if(ao4 != null){
      ao4.m1(fd0, fd1, ld0, fd0, ab1, ab2, ab3, ab4);
}
    Thought lo1 = Thought31.getInstance(fo0, fo1, ao1, ao2);
    for(int i0=0; i0<10; i0++){
if(ao3 != null){
          fb0 = ao3.m2(ao4, fo0, fo1, ao1, fd1, ld0, fd0, fd1, fb1, ab1, ab2, ab3);
}
if(ao2 != null){
          ao2.m2(ao3, ao4, fo0, fo1, ab4, fb0, fb1, ab1);
}
if(ao2 != null){
          ao1 = ao2.m4(ld0, fd0, fd1, ld0);
}
        ab2 = ab3 || ab4;
if(ao3 != null){
          fd0 = ao3.m3(ao4, fo0, fo1, ao1, fd1, ld0, fd0, fd1);
}
        boolean lb2 = true;
        ld0 *= -1;
        double ld3 = 184.20213141134346;
        Thought lo4 = Thought92.getInstance(ab4, fb0, fb1, lb2);
}
Thought.STACK_COUNTER++;
return ab2;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
boolean m2(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    boolean lb0 = true;
    double ld1 = 108.57011755452896;
if(ao3 != null){
      ao2 = ao3.m4();
}
    ad1 *= -1;
    ad2 = ad3 + ad4;
    boolean lb2 = true;
if(fo0 != null){
      ao4 = fo0.m4(fd0, fd1, ld1, ad1, lb0, lb2, ab1, ab2);
}
    ab3 = ab4 && fb0;
if(ao1 != null){
      fo1 = ao1.m4(ao2, ao3, ao4, fo0);
}
    Output.points[2][3] += ad2;
    double ld3 = 859.0331948870784;
    ad2 = ad3 + ad4;
    boolean lb4 = true;
    fd0 *= -1;
if(fo1 != null){
      fd1 = fo1.m3(ao1, ao2, ao3, ao4, ld1, ld3, ad1, ad2, fb0, fb1, lb0, lb2);
}
    double ld5 = 848.8108788009437;
if(fo1 != null){
      fo0 = fo1.m4(ao1, ao2, ao3, ao4, lb4, ab1, ab2, ab3);
}
    boolean lb6 = false;
    ad2 = ad3 + ad4;
if(fo1 != null){
      fo0 = fo1.m4(fd0, fd1, ld1, ld3);
}

Thought.STACK_COUNTER++;
return ab3;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return false;
} 
}
double m3() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fb1 = !fb0;
    fb1 = !fb0;
    fd0 = fd1 + fd0;
    Output.points[2][4] += fd1;
    fb1 = fb0 || fb1;
    fb0 = fb1 && fb0;
if(fo0 != null){
          fo0.m1(fo1, fo0, fo1, fo0, fd0, fd1, fd0, fd1);
}
    fd0 *= -1;
    if (fb1) {
        fb0 = !fb1;
        fb0 = fb1 && fb0;
if(fo0 != null){
          fo1 = fo0.m4(fb1, fb0, fb1, fb0);
}
        fb1 = fd1 > fd0;
        fd1 *= -1;
if(fo0 != null){
          fo1 = fo0.m4();
}
        Thought lo0 = Thought82.getInstance(fd0, fd1, fd0, fd1, fb0, fb1, fb0, fb1);
        } else if (fb0) {
}
Thought.STACK_COUNTER++;
return fd0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ab2 = fd1 < fd0;
    fd1 = fd0 - fd1;
    for(int i0=0; i0<10; i0++){
        Output.points[2][5] += fd0;
if(fo1 != null){
          fo1.m2(fo0, fo1, fo0, fo1);
}
        ab3 = ab4 || fb0;
        double ld0 = 131.61348597433116;
if(fo0 != null){
          fd0 = fo0.m3(fo1, fo0, fo1, fo0, fd1, ld0, fd0, fd1, fb1, ab1, ab2, ab3);
}
if(fo0 != null){
          fo1 = fo0.m4(fo1, fo0, fo1, fo0, ab4, fb0, fb1, ab1);
}
        ab2 = ab3 && ab4;
        fb0 = ld0 < fd0;
        fb1 = !ab1;
        ab2 = fd1 < ld0;
if(fo1 != null){
          fo1.m1(fd0, fd1, ld0, fd0);
}
        double ld1 = 159.02180196176238;
        Output.points[2][6] -= fd0;
        fd1 = ld0 + ld1;
        ab3 = fd0 < fd1;
        ab4 = ld0 > ld1;
        fd0 = fd1 + ld0;
        if (fb0) {
}}
Thought.STACK_COUNTER++;
return fd1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Output.points[2][7] -= ad1;
    Output.points[2][8] += ad2;
    boolean lb0 = false;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, ad3, ad4, fd0, fd1);
}
    double ld1 = 720.3013198979671;
    if (fb0) {
        boolean lb2 = true;
        fb0 = ld1 > ad1;
if(fo0 != null){
          ad2 = fo0.m3(fb1, lb2, lb0, fb0);
}
        fb1 = ad3 < ad4;
        lb2 = !lb0;
if(fo1 != null){
          fo1.m3();
}
        fd0 *= -1;
        fb0 = fd1 > ld1;
}
Thought.STACK_COUNTER++;
return ad1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Thought lo0 = Thought77.getInstance(ad1, ad2, ad3, ad4, ab1, ab2, ab3, ab4);
    Thought lo1 = Thought94.getInstance(fo0, fo1, fo0, fo1);
    fd0 *= -1;
    Thought lo2 = Thought20.getInstance(fo0, fo1, fo0, fo1, fd1, ad1, ad2, ad3, fb0, fb1, ab1, ab2);
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, ab3, ab4, fb0, fb1);
}
    ab1 = ad4 < fd0;
    ab2 = fd1 < ad1;
    Output.points[3][0] += ad2;
    Output.points[3][1] += ad3;
    ab3 = ab4 || fb0;
    fb1 = ad4 < fd0;
if(fo0 != null){
      fo0.m3(fd1, ad1, ad2, ad3);
}
    ad4 = fd0 - fd1;
    Thought lo3 = Thought92.getInstance(fo1, fo0, fo1, fo0, ad1, ad2, ad3, ad4);
if(fo1 != null){
      ab1 = fo1.m2(ab2, ab3, ab4, fb0);
}
    fd0 = fd1 - ad1;
    fb1 = ab1 || ab2;
    ab3 = ad2 < ad3;
    ab4 = ad4 > fd0;
    fb0 = fb1 || ab1;
if(fo0 != null){
      ab2 = fo0.m2();
}
    boolean lb4 = false;
if(fo1 != null){
      ab2 = fo1.m2(fd1, ad1, ad2, ad3, ab3, ab4, fb0, fb1);
}
    Output.points[3][2] -= ad4;
    lb4 = ab1 && ab2;
    ab3 = !ab4;
    Output.points[3][3] -= fd0;
    fb0 = fd1 < ad1;
if(fo0 != null){
      fo0.m1(fo1, fo0, fo1, fo0);
}

Thought.STACK_COUNTER++;
return ad2;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(Thought ao1, Thought ao2, Thought ao3, Thought ao4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Thought lo0 = Thought28.getInstance(ao2, ao3, ao4, fo0, fd0, fd1, fd0, fd1, fb0, fb1, fb0, fb1);
    fd0 = fd1 - fd0;
    Thought lo1 = Thought77.getInstance(fo1, ao1, ao2, ao3, fb0, fb1, fb0, fb1);
    fd1 *= -1;
        boolean lb2 = false;
    lb2 = fd0 > fd1;
    fb0 = fd0 > fd1;
    fb1 = !lb2;
    fb0 = fb1 && lb2;
    Thought lo3 = Thought70.getInstance(fd0, fd1, fd0, fd1);
    fb0 = fd0 < fd1;
    double ld4 = 801.3897630375102;
    ld4 *= -1;
    fb1 = fd0 > fd1;
if(fo0 != null){
      ao4 = fo0.m4(fo1, ao1, ao2, ao3, ld4, fd0, fd1, ld4);
}
    lb2 = !fb0;
    double ld5 = 72.78286145923627;
if(ao4 != null){
      ao4.m2(fb1, lb2, fb0, fb1);
}
    lb2 = fb0 || fb1;
    ld5 *= -1;
if(fo1 != null){
      fo0 = fo1.m4();
}
    fd0 = fd1 + ld4;
    boolean lb6 = true;
    lb2 = ld5 > fd0;
if(ao2 != null){
      ao1 = ao2.m4(fd1, ld4, ld5, fd0, lb6, fb0, fb1, lb2);
}

Thought.STACK_COUNTER++;
return fd1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ad1 = ad2 - ad3;
if(ao2 != null){
      ao1 = ao2.m4(ao3, ao4, fo0, fo1);
}
if(ao1 != null){
      fb1 = ao1.m2(ao2, ao3, ao4, fo0, ad4, fd0, fd1, ad1, fb0, fb1, fb0, fb1);
}
    fb0 = !fb1;
        ad2 = ad3 - ad4;
if(fo1 != null){
      fd0 = fo1.m3(ao1, ao2, ao3, ao4, fb0, fb1, fb0, fb1);
}
if(fo1 != null){
      fo0 = fo1.m4(fd1, ad1, ad2, ad3);
}
    Output.points[3][4] -= ad4;
    double ld0 = 582.4994643990188;
if(ao2 != null){
      ao1 = ao2.m4(ao3, ao4, fo0, fo1, ad4, fd0, fd1, ld0);
}
    Output.points[3][5] -= ad1;
    if (fb0) {
        ad2 *= -1;
}
Thought.STACK_COUNTER++;
return ad3;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd0 *= -1;
if(ao1 != null){
      ab2 = ao1.m2(ab3, ab4, fb0, fb1);
}
    ab1 = fd1 > fd0;
if(ao3 != null){
      ao2 = ao3.m4();
}
    ab2 = fd1 < fd0;
    boolean lb0 = true;
    ab2 = !ab3;
    fd1 = fd0 - fd1;
    ab4 = !fb0;
    fb1 = fd0 < fd1;
if(ao4 != null){
      ao4.m3(fd0, fd1, fd0, fd1, lb0, ab1, ab2, ab3);
}
    boolean lb1 = true;
    fd0 = fd1 + fd0;
if(fo0 != null){
      fo0.m3(fo1, ao1, ao2, ao3);
}
    boolean lb2 = false;
    fd1 = fd0 - fd1;
    fd0 *= -1;
    ab2 = fd1 > fd0;
    ab3 = ab4 || fb0;

Thought.STACK_COUNTER++;
return fd1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
double m3(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ad1 *= -1;
    ab1 = ab2 || ab3;
    for(int i0=0; i0<10; i0++){
if(ao3 != null){
          ao2 = ao3.m4(ao4, fo0, fo1, ao1, ad2, ad3, ad4, fd0, ab4, fb0, fb1, ab1);
}
if(ao3 != null){
          ao2 = ao3.m4(ao4, fo0, fo1, ao1, ab2, ab3, ab4, fb0);
}
        boolean lb0 = true;
        fb0 = fb1 && lb0;
if(ao2 != null){
          ab1 = ao2.m2(fd1, ad1, ad2, ad3);
}
        ab2 = ad4 < fd0;
        ab3 = ab4 || fb0;
        for(int i1=0; i1<10; i1++){
            Output.points[3][6] -= fd1;
if(ao4 != null){
              ao3 = ao4.m4(fo0, fo1, ao1, ao2, ad1, ad2, ad3, ad4);
}
if(ao3 != null){
              fb1 = ao3.m2(lb0, ab1, ab2, ab3);
}
if(fo0 != null){
              ao4 = fo0.m4();
}
if(fo1 != null){
              fd0 = fo1.m3(fd1, ad1, ad2, ad3, ab4, fb0, fb1, lb0);
}
            ab1 = ad4 > fd0;
if(ao2 != null){
              ao1 = ao2.m4(ao3, ao4, fo0, fo1);
}
if(ao2 != null){
              ao1 = ao2.m4(ao3, ao4, fo0, fo1, fd1, ad1, ad2, ad3, ab2, ab3, ab4, fb0);
}
            Thought lo1 = Thought94.getInstance(ao1, ao2, ao3, ao4, fb1, lb0, ab1, ab2);
if(fo0 != null){
              ab3 = fo0.m2(ad4, fd0, fd1, ad1);
}
            ab4 = ad2 > ad3;
}}
Thought.STACK_COUNTER++;
return ad4;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return 0.0;
} 
}
Thought m4() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd0 = fd1 - fd0;
    fb1 = fb0 && fb1;
if(fo1 != null){
      fd1 = fo1.m3(fo0, fo1, fo0, fo1, fd0, fd1, fd0, fd1);
}
    fb0 = !fb1;
if(fo1 != null){
      fo0 = fo1.m4(fb0, fb1, fb0, fb1);
}
    fd0 = fd1 + fd0;
    fd1 = fd0 - fd1;
    fb0 = !fb1;
if(fo0 != null){
      fb0 = fo0.m2();
}
    boolean lb0 = true;

Thought.STACK_COUNTER++;
return fo1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd0 = fd1 - fd0;
    boolean lb0 = false;
    fd1 = fd0 - fd1;
    fd0 = fd1 - fd0;
if(fo0 != null){
      fo0.m1(fd1, fd0, fd1, fd0, ab1, ab2, ab3, ab4);
}
    fd1 *= -1;
    fb0 = fd0 < fd1;
if(fo1 != null){
      fo1.m2(fo0, fo1, fo0, fo1);
}
    fd0 *= -1;
if(fo0 != null){
      fo0.m1(fo1, fo0, fo1, fo0, fd1, fd0, fd1, fd0, fb1, lb0, ab1, ab2);
}
if(fo1 != null){
      fo1.m3(fo0, fo1, fo0, fo1, ab3, ab4, fb0, fb1);
}
    double ld1 = 115.92716144005497;
if(fo0 != null){
      lb0 = fo0.m2(fd0, fd1, ld1, fd0);
}
    ab1 = ab2 || ab3;
    Thought lo2 = Thought75.getInstance(fo1, fo0, fo1, fo0, fd1, ld1, fd0, fd1);
    ab4 = fb0 && fb1;
    boolean lb3 = true;
    lb0 = lb3 || ab1;
    ab2 = ab3 || ab4;
if(fo1 != null){
      ld1 = fo1.m3(fb0, fb1, lb0, lb3);
}
if(fo0 != null){
      fo0.m3();
}
        fd0 = fd1 + ld1;
    fd0 = fd1 + ld1;
    Thought lo4 = Thought12.getInstance(fd0, fd1, ld1, fd0, ab1, ab2, ab3, ab4);
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0);
}
if(fo1 != null){
      fd1 = fo1.m3(fo0, fo1, fo0, fo1, ld1, fd0, fd1, ld1, fb0, fb1, lb0, lb3);
}
if(fo0 != null){
      fo0.m1(fo1, fo0, fo1, fo0, ab1, ab2, ab3, ab4);
}
    Output.points[3][7] -= fd0;

Thought.STACK_COUNTER++;
return fo1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(fo0 != null){
      ad1 = fo0.m3(ad2, ad3, ad4, fd0);
}
if(fo1 != null){
      fb0 = fo1.m2(fo0, fo1, fo0, fo1, fd1, ad1, ad2, ad3);
}
    Thought lo0 = Thought37.getInstance(fb1, fb0, fb1, fb0);
if(fo0 != null){
      ad4 = fo0.m3();
}
if(fo0 != null){
      fo1 = fo0.m4(fd0, fd1, ad1, ad2, fb1, fb0, fb1, fb0);
}
    fb1 = !fb0;
    fb1 = fb0 && fb1;
    fb0 = !fb1;
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0);
}
    Output.points[3][8] += ad3;
    boolean lb1 = false;
    double ld2 = 539.9237108324704;
    lb1 = fb0 && fb1;
if(fo1 != null){
      fo1.m2(fo0, fo1, fo0, fo1, ad3, ad4, fd0, fd1, lb1, fb0, fb1, lb1);
}
    Thought lo3 = Thought8.getInstance(fo0, fo1, fo0, fo1, fb0, fb1, lb1, fb0);
    if (fb1) {
        lb1 = fb0 || fb1;
        lb1 = ld2 > ad1;
if(fo0 != null){
          fo0.m3(ad2, ad3, ad4, fd0);
}
        fd1 = ld2 - ad1;
        fb0 = fb1 && lb1;
        fb0 = !fb1;
}
Thought.STACK_COUNTER++;
return fo1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    if (ab1) {
        double ld0 = 210.99753793502043;
        boolean lb1 = true;
if(fo0 != null){
          ab1 = fo0.m2(fo1, fo0, fo1, fo0, ld0, ad1, ad2, ad3);
}
        boolean lb2 = true;
if(fo1 != null){
          ad4 = fo1.m3(ab1, ab2, ab3, ab4);
}
        } else if (ab1) {
if(fo0 != null){
          ab2 = fo0.m2();
}
if(fo0 != null){
          fo1 = fo0.m4(fd1, ad1, ad2, ad3, ab3, ab4, fb0, fb1);
}
        } else {
        boolean lb3 = false;
        ad4 = fd0 + fd1;
if(fo1 != null){
          lb3 = fo1.m2(fo0, fo1, fo0, fo1);
}
if(fo0 != null){
          fo0.m1(fo1, fo0, fo1, fo0, ad1, ad2, ad3, ad4, ab1, ab2, ab3, ab4);
}
        fb0 = fd0 < fd1;
if(fo0 != null){
          fo1 = fo0.m4(fo1, fo0, fo1, fo0, fb1, lb3, ab1, ab2);
}
        ad1 = ad2 - ad3;
if(fo1 != null){
          ad4 = fo1.m3(fd0, fd1, ad1, ad2);
}
}
Thought.STACK_COUNTER++;
return fo0;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(Thought ao1, Thought ao2, Thought ao3, Thought ao4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, fd0, fd1, fd0, fd1);
}
    fb1 = fd0 > fd1;
    Output.points[4][0] -= fd0;
    Thought lo0 = Thought95.getInstance(fb0, fb1, fb0, fb1);
    fd1 *= -1;
if(ao2 != null){
      ao2.m3();
}
if(ao4 != null){
      ao3 = ao4.m4(fd0, fd1, fd0, fd1, fb0, fb1, fb0, fb1);
}
    fb0 = fb1 || fb0;
    fb1 = !fb0;
    boolean lb1 = true;
    Thought lo2 = Thought96.getInstance(fo0, fo1, ao1, ao2);
    fb0 = fd0 < fd1;

Thought.STACK_COUNTER++;
return ao3;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ad1 = ad2 + ad3;
if(ao2 != null){
      fb0 = ao2.m2(ao3, ao4, fo0, fo1, ad4, fd0, fd1, ad1, fb1, fb0, fb1, fb0);
}
if(ao2 != null){
      ao1 = ao2.m4(ao3, ao4, fo0, fo1, fb1, fb0, fb1, fb0);
}
    double ld0 = 497.5113693866287;
if(ao1 != null){
      ad1 = ao1.m3(ad2, ad3, ad4, fd0);
}
    fb1 = !fb0;
    fd1 = ld0 - ad1;
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, ad2, ad3, ad4, fd0);
}
    fd1 *= -1;
    ld0 = ad1 - ad2;
    Output.points[4][1] -= ad3;
    fb1 = ad4 < fd0;
    boolean lb1 = true;
    Thought lo2 = Thought69.getInstance(lb1, fb0, fb1, lb1);
if(ao2 != null){
      ao2.m3();
}
    fb0 = fb1 && lb1;
    fb0 = fd1 > ld0;
if(ao3 != null){
      ao3.m1(ad1, ad2, ad3, ad4, fb1, lb1, fb0, fb1);
}
    Thought lo3 = Thought4.getInstance(ao4, fo0, fo1, ao1);
        lb1 = !fb0;
if(ao2 != null){
      ao2.m3(ao3, ao4, fo0, fo1, fd0, fd1, ld0, ad1, fb1, lb1, fb0, fb1);
}
    lb1 = fb0 && fb1;
    lb1 = !fb0;

Thought.STACK_COUNTER++;
return ao1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(Thought ao1, Thought ao2, Thought ao3, Thought ao4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd0 = fd1 + fd0;
    fd1 = fd0 - fd1;
    Thought lo0 = Thought25.getInstance(ao2, ao3, ao4, fo0, ab1, ab2, ab3, ab4);
    double ld1 = 46.44833379994581;
    fb0 = fb1 && ab1;
    ld1 *= -1;
    fd0 = fd1 + ld1;
    fd0 *= -1;
if(ao1 != null){
      fo1 = ao1.m4(fd1, ld1, fd0, fd1);
}
if(ao3 != null){
      ao2 = ao3.m4(ao4, fo0, fo1, ao1, ld1, fd0, fd1, ld1);
}
if(ao2 != null){
      fd0 = ao2.m3(ab2, ab3, ab4, fb0);
}
    boolean lb2 = false;
if(ao3 != null){
      fd1 = ao3.m3();
}
    Thought lo3 = Thought26.getInstance(ld1, fd0, fd1, ld1, fb0, fb1, lb2, ab1);
    fd0 = fd1 + ld1;
    ab2 = !ab3;
    Thought lo4 = Thought79.getInstance(ao4, fo0, fo1, ao1);
    fd0 *= -1;
    ab4 = !fb0;
if(ao2 != null){
      fd1 = ao2.m3(ao3, ao4, fo0, fo1, ld1, fd0, fd1, ld1, fb1, lb2, ab1, ab2);
}
    boolean lb5 = true;
    double ld6 = 657.5294104075068;
if(ao1 != null){
      ld6 = ao1.m3(ao2, ao3, ao4, fo0, ab2, ab3, ab4, fb0);
}
if(fo1 != null){
          fb1 = fo1.m2(fd0, fd1, ld1, ld6);
}
    fd0 = fd1 + ld1;
    boolean lb7 = true;
    double ld8 = 632.3883258623562;

Thought.STACK_COUNTER++;
return ao1;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
Thought m4(Thought ao1, Thought ao2, Thought ao3, Thought ao4, double ad1, double ad2, double ad3, double ad4, boolean ab1, boolean ab2, boolean ab3, boolean ab4) throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    ad2 *= -1;
    ad3 *= -1;
    ab1 = ab2 || ab3;
    ab4 = ad4 < fd0;
if(ao2 != null){
      fd1 = ao2.m3(ao3, ao4, fo0, fo1, ad1, ad2, ad3, ad4);
}
    fd0 = fd1 - ad1;
    fb0 = !fb1;
    boolean lb0 = true;
    double ld1 = 744.6715817391269;
    lb0 = ad1 < ad2;
    boolean lb2 = true;
if(ao2 != null){
      ao1 = ao2.m4(lb2, ab1, ab2, ab3);
}
if(ao3 != null){
      ad3 = ao3.m3();
}

Thought.STACK_COUNTER++;
return ao4;
} catch (StackOverflowExc | NoClassDefFoundError e) { throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
return null;
} 
}
void accept(String color) throws CountDownExc {
switch (color) {
case "e":
empty();
break;
case "own":
own();
break;
case "other":
other();
break;
default:
throw new RuntimeException("Illegal color!!!");
}
}
void empty() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    fd0 = fd1 + fd0;
    fb1 = fd1 > fd0;
    fb0 = fd1 < fd0;
    fd1 = fd0 - fd1;
if(fo0 != null){
      fb1 = fo0.m2(fd0, fd1, fd0, fd1, fb0, fb1, fb0, fb1);
}
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0);
}
    Output.points[4][2] += fd0;
    fd1 = fd0 - fd1;
if(fo1 != null){
      fo1.m2(fo0, fo1, fo0, fo1, fd0, fd1, fd0, fd1, fb0, fb1, fb0, fb1);
}
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, fb0, fb1, fb0, fb1);
}
    fb0 = fb1 || fb0;
    fd0 = fd1 + fd0;
    fd1 = fd0 + fd1;
    fb1 = fd0 < fd1;
if(fo0 != null){
      fo0.m1(fd0, fd1, fd0, fd1);
}
    fb0 = !fb1;
    boolean lb0 = false;
if(fo1 != null){
      fd0 = fo1.m3(fo0, fo1, fo0, fo1, fd1, fd0, fd1, fd0);
}
    lb0 = !fb0;
    Output.points[4][3] -= fd1;
    fd0 *= -1;
    Output.points[4][4] += fd1;
    boolean lb1 = false;

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void own() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
if(fo1 != null){
      fo0 = fo1.m4(fb0, fb1, fb0, fb1);
}
    fd0 = fd1 - fd0;
    boolean lb0 = false;
    double ld1 = 311.11907551412594;
    double ld2 = 211.50451069755164;
    ld2 = fd0 - fd1;
if(fo0 != null){
      fo0.m2();
}
    ld1 = ld2 - fd0;
    fd1 *= -1;
    lb0 = fb0 || fb1;
if(fo0 != null){
      fo1 = fo0.m4(ld1, ld2, fd0, fd1, lb0, fb0, fb1, lb0);
}
if(fo0 != null){
      fo1 = fo0.m4(fo1, fo0, fo1, fo0);
}
if(fo1 != null){
      fb0 = fo1.m2(fo0, fo1, fo0, fo1, ld1, ld2, fd0, fd1, fb1, lb0, fb0, fb1);
}
if(fo0 != null){
      fo0.m3(fo1, fo0, fo1, fo0, lb0, fb0, fb1, lb0);
}
    fb0 = fb1 || lb0;
if(fo1 != null){
      fb0 = fo1.m2(ld1, ld2, fd0, fd1);
}

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
void other() throws CountDownExc {
Thought.STACK_COUNTER--;
if(Thought.STACK_COUNTER <=0){
throw new StackOverflowExc();
}
try {
if (false) { throw new CountDownExc(-1); }
    Output.points[4][5] += fd0;
    fb0 = fb1 || fb0;
if(fo1 != null){
          fo0 = fo1.m4(fo0, fo1, fo0, fo1, fd1, fd0, fd1, fd0);
}
if(fo0 != null){
      fd1 = fo0.m3(fb1, fb0, fb1, fb0);
}
if(fo1 != null){
      fd0 = fo1.m3();
}
    Output.points[4][6] -= fd1;
    boolean lb0 = false;
    fb0 = fb1 || lb0;
if(fo0 != null){
      fd0 = fo0.m3(fd1, fd0, fd1, fd0, fb0, fb1, lb0, fb0);
}
    double ld1 = 545.1947795846902;
    fb1 = !lb0;
if(fo1 != null){
      fd0 = fo1.m3(fo0, fo1, fo0, fo1);
}
    fb0 = fd1 > ld1;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, fd0, fd1, ld1, fd0, fb1, lb0, fb0, fb1);
}
    Output.points[4][7] += fd1;
    lb0 = fb0 || fb1;
    lb0 = ld1 < fd0;
    Thought lo2 = Thought16.getInstance(fo0, fo1, fo0, fo1, fb0, fb1, lb0, fb0);
    fb1 = lb0 && fb0;
    Output.points[4][8] -= fd1;
if(fo1 != null){
      fo0 = fo1.m4(ld1, fd0, fd1, ld1);
}
    fd0 = fd1 + ld1;
    Output.points[5][0] -= fd0;
if(fo1 != null){
      fo0 = fo1.m4(fo0, fo1, fo0, fo1, fd1, ld1, fd0, fd1);
}

Thought.STACK_COUNTER++;
} catch (StackOverflowExc | NoClassDefFoundError e) { 
throw new CountDownExc(50);
} catch (CountDownExc e) {
if(!e.surfaced()) {
e.dec();
throw e;
}
} 
}
}
